...
class EnglishEntryhijacker extends StatefulWidget {
  const EnglishEntryhijacker({super.key});

  @override
  State<EnglishEntryhijacker> createState() => _EnglishEntryhijackerState();
}

class _EnglishEntryhijackerState extends State<EnglishEntryhijacker> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: hijacker (Derived forms: hijackers)
1. A holdup man who stops a vehicle and steals from it (- highjacker, highwayman, road agent)
 
2. Someone who uses force to take over a vehicle (especially an aeroplane) in order to reach an alternative destination (- highjacker)
""",
  );

  final String keyword = "hijacker";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""hijacker""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""Airline passengers tried to overpower the hijackers.""");
  }
  ...
  Future<void> speaksentence2(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""Hijackers crashed a plane into the Pentagon.""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const double startSecondsend = 000000000000000000000000000567;
    const String videoIdone = 'https://youtu.be/1Y1kJpHBn50?t=';
    const double startSecondsone = 000000000000000000000000000476;
    const String videoIdtwo = 'https://youtu.be/0jC69NDjCEU?t=';
    const double startSecondstwo = 000000000000000000000000000185;
    const String videoIdthree = 'https://youtu.be/8yStwJOpvd8?t=';
    const double startSecondsthree = 000000000000000000000000000564;
    const String videoIdfour = 'https://youtu.be/vbz83-nAXrY?t=';
    const double startSecondsfour = 000000000000000000000000000170;
    const String videoIdfive = 'https://youtu.be/Jw_ysaoVlt4?t=';
    const double startSecondsfive = 000000000000000000000000000747;
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """hijacker""",
                      // alsoEnglishWord: "also: hijacker",
                      britshText: """IpaUK: /ˈhaɪdʒækə(r)/""",
                      americanText: """IpaUS: /ˈhaɪdʒækər/""",
...
کوردی: فڕۆکەگر، فڕۆکەڕفێن، فڕۆکەدز
"""),
// With short examples define "hijacker", please follow LX instructions
                    const DefinitionKurdish(
                        text:
                            """١. (ناو) کەسێک کە فڕۆکەیەک یان پێڕۆیەکی دیکە دەڕفێنێت"""),
                    SentencesRow(
                      englishText:
                          """Airline passengers tried to overpower the hijackers.""",
                      kurdishText:
                          """گەشتیارانی هێڵە ئاسمانییەکە هەوڵیان سەرکەون بەسەر ڕفێنەرەکاندا.""",
                    ...
                    SentencesRow(
                      englishText:
                          """Hijackers crashed a plane into the Pentagon.""",
                      kurdishText:
                          """ڕفێنەرەکان فڕۆکەیەکیان کێشا بە پنتاگۆندا.""",

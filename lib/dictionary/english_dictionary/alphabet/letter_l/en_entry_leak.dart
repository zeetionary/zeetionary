...
class EnglishEntryleak extends StatefulWidget {
  const EnglishEntryleak({super.key});

  @override
  State<EnglishEntryleak> createState() => _EnglishEntryleakState();
}

class _EnglishEntryleakState extends State<EnglishEntryleak> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: leak (Derived forms: leaks)
1. The discharge of a fluid from some container (- escape, leakage, outflow)
"he had to clean up the leak";
 
2. An accidental hole that allows something (fluid or light etc.) to enter or escape
"one of the tires developed a leak"
 
3. Unauthorized (especially deliberate) disclosure of confidential information (- news leak)
 
4. [informal] A euphemism for urination (- wetting, making water, passing water)
"he had to take a leak";
 
5. Soft watery rot in fruits and vegetables caused by fungi

- Verb: leak (Derived forms: leaked, leaks, leaking)
...
""",
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""The tank had leaked a small amount of water.""");
  }
  ...
  Future<void> speaksentence2(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""The roof was leaking.""");
  }
  ...
  Future<void> speaksentence3(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""Gas was leaking from one of the pipes.""");
  }
  ...
  Future<void> speaksentence4(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""The contents of the report were leaked to the press.""");
  }
  ...
  Future<void> speaksentence5(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""Fortunately, we spotted the leak in time.""");
  }
  ...
  Future<void> speaksentence6(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""The boat had a small leak.""");
  }
  ...
  Future<void> speaksentence7(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""If you have no idea where the gas leak is coming from, it is always best to turn the complete system off.""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const String videoIdend = 'https://youtu.be/-CJ4eUhE3Lw?t=766';
    const double startSecondsend = 0;
    const String videoIdone = 'https://youtu.be/AF8d72mA41M?t=147';
    const double startSecondsone = 0;
    const String videoIdtwo = 'https://youtu.be/qWAagS_MANg?t=277';
    const double startSecondstwo = 0;
    const String videoIdthree = 'https://youtu.be/cqidD7kVnxY?t=235';
    const double startSecondsthree = 0;
    const String videoIdfour = 'https://youtu.be/oMF6pIglmmk?t=826';
    const double startSecondsfour = 0;
    const String videoIdfive = 'https://youtu.be/CaaJyRvvaq8?t=434';
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """leak""",
                      // alsoEnglishWord: "also: leak",
                      britshText: """IpaUK: leak""",
                      americanText: """IpaUS: leak""",
...
کوردی: درز، کون، قەڵەش،	لێچوون، دادان، داچۆڕان، لێڤەچوون، لێ‌ڕۆیین، وەرڤین، دەڵاندن، دڵۆپەکردن، دزە، لێهاتن،	لە قاودان، ڕاستی لێ‌وتن
"""),
// With short examples define "leak", please follow LX instructions
                    const DefinitionKurdish(text: """١. (کردار) ئەوەی شتێک ڕێگا بدات شلە و گاز پێیدا تێپەڕێت"""),
                    SentencesRow(
                      englishText: """The tank had leaked a small amount of water.""",
                      kurdishText: """تانکییەکە ڕێژەیەکی کەمی لە ئاو دەردابوو.""",
                    ...
                    SentencesRow(
                      englishText: """The roof was leaking.""",
                      kurdishText: """بنبانەکە تکەی دەکرد.""",
                      onPressedBritish: () => speaksentence2("en-GB"),
                      onPressedAmerican: () => speaksentence2("en-US"),
                    ),
                    const DividerDefinition(),
                    const DefinitionKurdish(text: """٢. (کردار) چوونە دەرەوە یان ناوەوە لە کونێکی بچووکەوە"""),
                    const DividerSentences(),
                    SentencesRow(
                      englishText: """Gas was leaking from one of the pipes.""",
                      kurdishText: """گاز لە یەکێک لە بۆرییەکانەوە هاتووچۆی دەکرد.""",
                      onPressedBritish: () => speaksentence3("en-GB"),
                      onPressedAmerican: () => speaksentence3("en-US"),
                    ),
                    const DividerDefinition(),
                    const DefinitionKurdish(text: """٣. (کردار) پێدانی زانیاری بە خەڵکی، بۆ نموونە گوتن بە پەیامنێرێک"""),
                    const AlsoEnglishckb(word: "ھەروەھا: disclose"),
                    SentencesRow(
                      englishText: """The contents of the report were leaked to the press.""",
                      kurdishText: """ناوەڕۆکی ڕاپۆرتەکە بە میدیا گوترا.""",
                      onPressedBritish: () => speaksentence4("en-GB"),
                      onPressedAmerican: () => speaksentence4("en-US"),
                    ),
                    const DividerDefinition(),
                    const DefinitionKurdish(text: """٤. (ناو) کونێکی بچووک کە دەهێڵێت شلە و گاز پێیدا هاتووچۆ بکات بە ڕێکەوت"""),
                    const DividerSentences(),
                    SentencesRow(
                      englishText: """Fortunately, we spotted the leak in time.""",
                      kurdishText: """خۆشبەختانە زوو درزەکەمان دۆزییەوە.""",
                    ...
                    const DividerSentences(),
                    SentencesRow(
                      englishText: """The boat had a small leak.""",
                      kurdishText: """بەلەمەکە درزێکی بچووکی تێدابوو.""",
                      onPressedBritish: () => speaksentence6("en-GB"),
                      onPressedAmerican: () => speaksentence6("en-US"),
                    ),
                    const DividerDefinition(),
                    const DefinitionKurdish(text: """٥. (ناو) شلە و گاز کە بە شتێکدا هاتووچۆ دەکات"""),
                    const DividerSentences(),
                    SentencesRow(
                      englishText: """If you have no idea where the gas leak is coming from, it is always best to turn the complete system off.""",
                      kurdishText: """ئەگەر هیچ نازانیت کە لێچوونی گازەکە لە کوێوە دێت، هەمیشە باشترین شت وایە کە سیستەمەکە بە تەواوی بکوژێنیتەوە.""",

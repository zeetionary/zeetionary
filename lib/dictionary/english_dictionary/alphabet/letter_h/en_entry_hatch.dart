...
class EnglishEntryhatch extends StatefulWidget {
  const EnglishEntryhatch({super.key});

  @override
  State<EnglishEntryhatch> createState() => _EnglishEntryhatchState();
}

class _EnglishEntryhatchState extends State<EnglishEntryhatch> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Verb: hatch (Derived forms: hatching, hatches, hatched)
1. Emerge from the egg
"young birds, fish, and reptiles hatch"
 
2. Devise or invent (- think up, think of, dream up, concoct, hit on)
...
5. Sit on (eggs) (- brood, cover, incubate)

- Noun: hatch (Derived forms: hatches)
1. The production of young from an egg (- hatching)
 
2. Shading consisting of multiple crossing lines (- hatching, crosshatch, hachure)
 
3. A sloping rear car door that is lifted to open (- hatchback, hatchback door, liftgate [US])
 
4. A movable barrier covering a hatchway
 
5. An entrance equipped with a hatch; especially a passageway between decks of a ship (- hatchway, opening, scuttle)
""",
  );

  final String keyword = "hatch";
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""Ten chicks hatched (out) this morning.""");
  }
  ...
  Future<void> speaksentence2(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""The eggs are about to hatch.""");
  }
  ...
  Future<void> speaksentence4(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""The female must find a warm place to hatch her eggs.""");
  }
  ...
  Future<void> speaksentence6(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""Have you been hatching up a deal with her?""");
  }
  ...
  Future<void> speaksentence9(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""Leave the hatch open.""");
  }
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """hatch""",
                      // alsoEnglishWord: "also: hatch",
                      britshText: """IpaUK: /hætʃ/""",
                      americanText: """IpaUS: /hætʃ/""",
...
کوردی: دەربیجە، دەلاقە، کولاوکە، کڵاوڕۆژنە،	(کەشتی) درگا، دەرکە، نیوەدرگا، دەلاقە
"""),
// With short examples define "hatch", please follow LX instructions
                    const DefinitionKurdish(
                        text: """١. (کردار) دەرهاتنی بێچوو لە هێلکە"""),
                    SentencesRow(
                      englishText: """Ten chicks hatched (out) this morning.""",
                      kurdishText: """جووجکەکە ئەم بەیانییە هەڵهات.""",
                    ...
                    const DefinitionKurdish(
                        text:
                            """٢. (کردار) شکانی هێلکە بۆ ئەوەی بێچوو لە ناوی بێتە دەرەوە"""),
                    SentencesRow(
                      englishText: """The eggs are about to hatch.""",
                      kurdishText: """هێلکەکان خەریکە دەترووکێن.""",
                      onPressedBritish: () => speaksentence2("en-GB"),
                      onPressedAmerican: () => speaksentence2("en-US"),
                    ),
                    SentencesRow(
                      englishText: """The eggs are about to hatch.""",
                      kurdishText: """چەندی پێدەچێت هێلکەکان بترووکێن؟""",
                      onPressedBritish: () => speaksentence3("en-GB"),
                      onPressedAmerican: () => speaksentence3("en-US"),
                    ),
                    const DividerDefinition(),
                    const DefinitionKurdish(
                        text: """٣. (کردار) کڕکەوتن لەسەر هێلکە"""),
                    SentencesRow(
                      englishText:
                          """The female must find a warm place to hatch her eggs.""",
                      kurdishText:
                          """مێینەکە دەبێت شوێنێکی گەرم بدۆزێتەوە بۆ کڕکەوتن لەسەر هێلکەکانی.""",
                    ...
                    const DefinitionKurdish(
                        text:
                            """٤. (کردار) داڕشتی پلان یان بیرۆکەیەک، بە تایبەتی بە نهێنی"""),
                    SentencesRow(
                      englishText:
                          """Have you been hatching up a deal with her?""",
                      kurdishText: """هیچ ڕێککەوتنێکت داڕشتووە لەگەڵی؟""",
                    ...
                    SentencesRow(
                      englishText:
                          """Rebel MPs are hatching a secret plot to oust the prime minister.""",
                      kurdishText:
                          """ئەندامە پەرلەمانە یاخییەکان پلانێکی نهێنی دادەڕێژن بۆ دەرکردنی سەرۆک وەزیران.""",
                      onPressedBritish: () => speaksentence7("en-GB"),
                      onPressedAmerican: () => speaksentence7("en-US"),
                    ),
                    const DividerDefinition(),
                    CustomRichText(
                      textBeforeLink: "٥. بڕوانە لە ",
                      linkText: "hatchway",
                      textAfterLink: "",
                      onTap: () {
                        Routemaster.of(context).push("/english-hatch/hatchway");
                      },
                    ),
                    const DividerDefinition(),
                    const DefinitionKurdish(
                        text:
                            """٦. (ناو) دەلاقەی نێوان دوو ژوور، بە تایبەتی چێشتخانە و ژووری نانخواردن بۆ گواستنەوەی خواردن"""),
                    SentencesRow(
                      englishText:
                          """She opened the serving hatch and put the soup on the counter.""",
                      kurdishText:
                          """تاقی خواردنەکەی کردەوە و شۆرباکەی خستە سەر مێزەکە.""",
                      onPressedBritish: () => speaksentence8("en-GB"),
                      onPressedAmerican: () => speaksentence8("en-US"),
                    ),
                    const DividerDefinition(),
                    const DefinitionKurdish(
                        text: """٧. (ناو) کونێک لە زەوی یان بنبانێک"""),
                    SentencesRow(
                      englishText: """Leave the hatch open.""",
                      kurdishText: """دەلاقەکە بە کراوەیی بهێڵەرەوە.""",
...
// 188888880002200
const String _videoIdend = 'https://youtu.be/72KnZ_dfC0w?t=';
const double _startSecondsend = 00000000000000000000000000031;
const String _videoIdone = 'https://youtu.be/5eqFuiXowo4?t=';
const double _startSecondsone = 00000000000000000000000000075;
const String _videoIdtwo = 'https://youtu.be/vuylMr_jqG0?t=';
const double _startSecondstwo = 00000000000000000000000000036;
const String _videoIdthree = 'https://youtu.be/0T-ZOfuGB2U?t=';
const double _startSecondsthree = 00000000000000000000000000082;
const String _videoIdfour = 'https://youtu.be/f4iNtTlDeaE?t=';
const double _startSecondsfour = 00000000000000000000000000014;
const String _videoIdfive = 'https://youtu.be/UkqNUdbUpJQ?t=';

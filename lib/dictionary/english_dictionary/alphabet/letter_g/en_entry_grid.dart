...
class EnglishEntrygrid extends StatefulWidget {
  const EnglishEntrygrid({super.key});

  @override
  State<EnglishEntrygrid> createState() => _EnglishEntrygridState();
}

class _EnglishEntrygridState extends State<EnglishEntrygrid> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: grid (Derived forms: grids)
1. A pattern of regularly spaced horizontal and vertical lines
 
2. A system of high tension cables by which electrical power is distributed throughout a region (- power system, power grid)
 
2. A perforated or corrugated metal plate used in a storage battery as a conductor and support for the active material (- storage-battery grid)
 
3. An electrode placed between the cathode and anode of a vacuum tube to control the flow of electrons through the tube (- control grid)
 
4. A cooking utensil of parallel metal bars; used to grill fish or meat (- gridiron, grill)
 
5. (cartography) a method of marking off maps into areas

- Verb: grid (Derived forms: grided, griding, grids)
1. Mark with or put into a grid
""",
  );

  final String keyword = "grid";
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""We superimposed a grid over the image.""");
  }
  ...
  Future<void> speaksentence3(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""The window had a metal grid.""");
  }
  ...
  Future<void> speaksentence4(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""The grid reference is C8.""");
  }
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """grid""",
                      // alsoEnglishWord: "also: grid",
                      britshText: """IpaUK: /ɡrɪd/""",
                      americanText: """IpaUS: /ɡrɪd/""",
...
کوردی: تۆڕ،	نەردە،	تۆڕی خانەخانە یان چوارگۆشاوی، شیش‌بەند، میل یان شیشی تۆڕی، تۆڕی چوارخانەیی،	تۆڕی ئێلیکتریسیتە
"""),
                    const DefinitionKurdish(
                        text:
                            """١. (ناو) شێوەیەک کە لە هێڵی ڕاست پێکهاتووە و یەکتری دەبڕن"""),
                    SentencesRow(
                      englishText: """We superimposed a grid over the image.""",
                      kurdishText: """خشتەیەک دانا لەسەر وێنەکە.""",
                    ...
                    SentencesRow(
                      englishText:
                          """The artist drew a set of grid lines over the area to be painted.""",
                      kurdishText:
                          """هونەرمەندەکە ڕیزێک لە هێڵی کێشا بەسەر ناوچەکە بۆ ئەوەی بۆیە بکرێت.""",
                    ...
                    SentencesRow(
                      ...
                      kurdishText: """پەنجەرەکە شیشبەندێکی هەبوو.""",
                    ...
                    SentencesRow(
                      englishText:
                          """The development of the country’s electricity grid took years.""",
                      kurdishText:
                          """گەشەپێدانی سیستەمی کارەبای وڵاتەکە چەندین ساڵی پێچوو.""",
                    ...
                    const DefinitionKurdish(
                        text:
                            """٦. (ناو) کۆمەڵێک هێڵ کە خاڵی سەرەتای پێشبڕکێیەک دیاری دەکات"""),
                    const AlsoEnglishckb(word: "ھەروەھا: starying grid"),
                    SentencesRow(
                      englishText:
                          """The 20-year-old didn't get the best of starts off the grid.""",
                      kurdishText:
                          """وەرزشکارە ٢٠ ساڵانەکە باشترین دەستپێکی نەبوو لە خاڵی سەرەتاوە.""",
...
// 188888880002200
const String _videoIdend = 'https://youtu.be/_45zjnjrFOA?t=';
...
const String _videoIdthree = 'https://youtu.be/LfR4QfmwTAc?t=';
const double _startSecondsthree = 000000000000000000000000000239;
const String _videoIdfour = 'https://youtu.be/GVsUOuSjvcg?t=';
const double _startSecondsfour = 000000000000000000000000000291;
const String _videoIdfive = 'https://youtu.be/0Anx3VQ2dcc?t=';

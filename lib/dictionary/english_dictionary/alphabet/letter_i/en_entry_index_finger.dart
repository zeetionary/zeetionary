...
class EnglishEntryindexfinger extends StatefulWidget {
  const EnglishEntryindexfinger({super.key});

  @override
  State<EnglishEntryindexfinger> createState() =>
      _EnglishEntryindexfingerState();
}

class _EnglishEntryindexfingerState extends State<EnglishEntryindexfinger> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: index finger (Derived forms: index fingers)
1. The finger next to the thumb (- index, forefinger)
""",
  );

  final String keyword = "index finger";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""index finger""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""He raised his index finger to ask a question.""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const double startSecondsend = 000000000000000000000000000749;
    ...
    const double startSecondstwo = 000000000000000000000000000267;
    const String videoIdthree = 'https://youtu.be/9hvjBi4PKWA?t=';
    const double startSecondsthree = 000000000000000000000000000123;
    const String videoIdfour = 'https://youtu.be/yRFPjBuRRCg?t=';
    const double startSecondsfour = 0000000000000000000000000001280;
    const String videoIdfive = 'https://youtu.be/0FcwzMq4iWg?t=';
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """index finger""",
                      // alsoEnglishWord: "also: index finger",
                      britshText: """IpaUK: /ˈɪndeks fɪŋɡə(r)/""",
                      americanText: """IpaUS: /ˈɪndeks fɪŋɡər/""",
...
کوردی: دۆشاومژە، قامکی شادە، قامکی نیشانە
"""),
                    const DefinitionKurdish(
                        text:
                            """١. (ناو) پەنجەی تەنیشت پەنجە قەڵەو و بچووکەکەی دەست"""),
                    SentencesRow(
                      englishText:
                          """He raised his index finger to ask a question.""",
                      kurdishText:
                          """پەنجەی دۆشاوە مژەکەی بەرز کردەوە بۆ پرسینی پرسیارێک.""",

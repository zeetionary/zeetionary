...
class EnglishEntryinsulate extends StatefulWidget {
  const EnglishEntryinsulate({super.key});

  @override
  State<EnglishEntryinsulate> createState() => _EnglishEntryinsulateState();
}

class _EnglishEntryinsulateState extends State<EnglishEntryinsulate> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Verb: insulate (Derived forms: insulating, insulates, insulated)
1. Protect from heat, cold, or noise by surrounding with insulating material
"We had his bedroom insulated before winter came"
 
2. Separate in order to protect or prevent interaction (- isolate)
"They insulated the political prisoners from the other inmates";
""",
  );

  final String keyword = "insulate";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""insulate""");
  }
  ...
  Future<void> speaksentence2(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""Children should be insulated from the horrors of war.""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const double startSecondsend = 000000000000000000000000000658;
    const String videoIdone = 'https://youtu.be/GcdB5bFwio4?t=';
    const double startSecondsone = 000000000000000000000000000207;
    const String videoIdtwo = 'https://youtu.be/HaP_OeRuNew?t=';
    const double startSecondstwo = 000000000000000000000000000128;
    const String videoIdthree = 'https://youtu.be/Q-nWA0WeF98?t=';
    const double startSecondsthree = 000000000000000000000000000641;
    const String videoIdfour = 'https://youtu.be/brofphmC7GI?t=';
    const double startSecondsfour = 0000000000000000000000000001596;
    const String videoIdfive = 'https://youtu.be/f8euUepDo5w?t=';
    const double startSecondsfive = 000000000000000000000000000535;
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """insulate""",
                      // alsoEnglishWord: "also: insulate",
                      britshText: """IpaUK: /ˈɪnsjuleɪt/""",
                      americanText: """IpaUS: /ˈɪnsəleɪt/""",
...
کوردی: 
"""),
// With short examples define "insulate", please follow LX instructions
                    const DefinitionKurdish(
                        text:
                            """١. (کردار) پاراستنی شتێک بە مادەیەک کە ڕێگا دەگرێت لەوەی گەرمی، دەنگ، کارەبا، هتد بەناویدا تێپەڕێت"""),
                    SentencesRow(
                      englishText:
                          """Home owners are being encouraged to insulate their homes to save energy.""",
                      kurdishText:
                          """خاوەن خانووەکان هان دەدرێن خانووەکانیان عەزل بکەن بۆ پاراستنی وزە.""",
                    ...
                    SentencesRow(
                      ...
                      kurdishText:
                          """منداڵان دەبێت بپارێزرێن لە مەترسییەکانی جەنگ.""",

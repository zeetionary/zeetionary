...
class EnglishEntryintray extends StatefulWidget {
  const EnglishEntryintray({super.key});

  @override
  State<EnglishEntryintray> createState() => _EnglishEntryintrayState();
}

class _EnglishEntryintrayState extends State<EnglishEntryintray> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: in tray (Derived forms: in-trays)
1. A wood or metal receptacle placed on your desk to hold your incoming material (- in-basket, inbox)
""",
  );

  final String keyword = "in tray";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""in tray""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""She had a full in tray to deal with when she got back.""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const String videoIdone = 'https://youtu.be/StgMJMn80t8?t=';
    const double startSecondsone = 000000000000000000000000000779;
    const String videoIdtwo = 'https://youtu.be/RZvOKgsPUrs?t=';
    const double startSecondstwo = 00000000000000000000000000066;
    const String videoIdthree = 'https://youtu.be/QiiNBtPOTH4?t=';
    const double startSecondsthree = 000000000000000000000000000452;
    const String videoIdfour = 'https://youtu.be/Tp6k-QHI_co?t=';
    const double startSecondsfour = 000000000000000000000000000319;
    const String videoIdfive = 'https://youtu.be/WpgsITqoDXQ?t=';
    const double startSecondsfive = 0000000000000000000000000001454;
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """in tray""",
                      // alsoEnglishWord: "also: in tray",
                      britshText: """IpaUK: /ˈɪn treɪ/""",
                      americanText: """IpaUS: /ˈɪn treɪ/""",
...
کوردی: 
"""),
// With short examples define "in tray", please follow LX instructions
                    const DefinitionKurdish(
                        text:
                            """١. (ناو) بەشێکی سەر دێسکەکەت کە ئەو نامانەی تێدایە کە وەڵام نەدراون یان نەخوێندراون"""),
                    SentencesRow(
                      englishText:
                          """She had a full in tray to deal with when she got back.""",
                      kurdishText:
                          """کە گەڕایەوە جێگایەکی پڕ لە نامەی گەشتووی کە وەڵامیان بداتەوە.""",

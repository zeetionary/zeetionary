...
class EnglishEntryhonk extends StatefulWidget {
  const EnglishEntryhonk({super.key});

  @override
  State<EnglishEntryhonk> createState() => _EnglishEntryhonkState();
}

class _EnglishEntryhonkState extends State<EnglishEntryhonk> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Verb: honk (Derived forms: honked, honking, honks)
1. Make a strident noise (- blare, beep, claxon, toot, parp [informal], klaxon)
"The horns of the taxis honked";
 
2. Use the horn of a car (- claxon, parp [informal], klaxon)
 
3. Cry like a goose (- cronk)
"The geese were honking";
 
4. [Brit, informal] Eject the contents of the stomach through the mouth (- vomit, vomit up, purge, sick, cat [informal], be sick, disgorge, regorge [archaic], retch, barf [informal], spew [informal], spue [archaic], chuck [informal], upchuck [N. Amer, informal], regurgitate, throw up, vom [informal], keck [informal], hurl [informal], chuck up [Brit, informal], chunder [Brit, informal], boke [UK, dialect], boak [UK, dialect])
"After drinking too much, the students honked";

- Noun: honk (Derived forms: honks)
...
""",
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""The honk startled the pedestrians on the sidewalk.""");
  }
  ...
  Future<void> speaksentence2(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""Why did he honk at me?""");
  }
  ...
  Future<void> speaksentence3(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""People honked their horns as they drove past.""");
  }
  ...
  Future<void> speaksentence4(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""He honked (up) all over the floor.""");
  }
  ...
  Future<void> speaksentence5(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""A flock of geese honked near the pond.""");
  }
  ...
  Widget build(BuildContext context) {
    // 188888880002200
    const String videoIdend = 'https://youtu.be/xRyPWG9PTm4?t=';
    const double startSecondsend = 000000000000000000000000000758;
    const String videoIdone = 'https://youtu.be/2WV_0hiuVOg?t=';
    const double startSecondsone = 0000000000000000000000000001327;
    const String videoIdtwo = 'https://youtu.be/cvoijkA75wM?t=';
    const double startSecondstwo = 000000000000000000000000000414;
    const String videoIdthree = 'https://youtu.be/DoOOPelMaYY?t=';
    const double startSecondsthree = 00000000000000000000000000059;
    const String videoIdfour = 'https://youtu.be/waBWX2dBGOI?t=';
    const double startSecondsfour = 00000000000000000000000000036;
    const String videoIdfive = 'https://youtu.be/JMcmlEfLRa8?t=';
    const double startSecondsfive = 000000000000000000000000000288;
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """honk""",
                      // alsoEnglishWord: "also: honk",
                      britshText: """IpaUK: /hɒŋk/""",
                      americanText: """IpaUS: /hɑːŋk/""",
...
کوردی: (قاز) قاڕوقیڕ، قاڕە، گارەگار، قاژەقاژ، هۆرن
"""),
                    const DefinitionKurdish(
                        text: """١. (ناو) دەنگی قاز یان هۆرنی ئۆتۆمبێل"""),
                    SentencesRow(
                      englishText:
                          """The honk startled the pedestrians on the sidewalk.""",
                      kurdishText:
                          """هۆرنەکە پیادەڕەوەکانی ترساند لەسەر ڕەسیفەکە.""",
                      onPressedBritish: () => speaksentence1("en-GB"),
                      onPressedAmerican: () => speaksentence1("en-US"),
                    ),
                    const DividerDefinition(),
                    const DefinitionKurdish(text: """٢. (کردار) هۆرن لێدان"""),
                    const AlsoEnglishckb(word: "ھەروەھا: hoot"),
                    SentencesRow(
                      englishText: """Why did he honk at me?""",
                      kurdishText: """بۆ هۆرنت بۆم لێدا؟""",
                    ...
                    SentencesRow(
                      ...
                      kurdishText: """خەڵکی هۆرنەکانیان لێدەدا کە تێدەپەڕین.""",
                    ...
                    SentencesRow(
                      ...
                      kurdishText:
                          """کۆمەڵێک قاز دەیانقیڕاند لە نزیکی زۆنگاوەکە.""",

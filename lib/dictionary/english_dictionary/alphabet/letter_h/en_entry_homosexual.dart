...
class EnglishEntryhomosexual extends StatefulWidget {
  const EnglishEntryhomosexual({super.key});

  @override
  State<EnglishEntryhomosexual> createState() => _EnglishEntryhomosexualState();
}

class _EnglishEntryhomosexualState extends State<EnglishEntryhomosexual> {
  @override
  void initState() {
    super.initState();
...
2. Of or relating to homosexual men or women (- same-sex, gay)

- Noun: homosexual (Derived forms: homosexuals)
1. Someone who is sexually attracted to persons of the same sex (- homophile [archaic], gay)
""",
  );

  final String keyword = "homosexual";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""homosexual""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""The homosexual woman shared her experiences at the event.""");
  }
  ...
  Future<void> speaksentence2(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""He came out as a homosexual after his mother died.""");
  }
  ...
  Widget build(BuildContext context) {
    // 188888880002200
    const String videoIdend = 'https://youtu.be/9-GRzu6zbS0?t=';
    const double startSecondsend = 00000000000000000000000000072;
    const String videoIdone = 'https://youtu.be/rQM4-zbS9fc?t=';
    const double startSecondsone = 000000000000000000000000000583;
    const String videoIdtwo = 'https://youtu.be/f5jwTft6654?t=';
    const double startSecondstwo = 000000000000000000000000000171;
    const String videoIdthree = 'https://youtu.be/qSmyimnMOBo?t=';
    const double startSecondsthree = 0000000000000000000000000001196;
    const String videoIdfour = 'https://youtu.be/Un1gfVckcVw?t=';
    const double startSecondsfour = 000000000000000000000000000541;
    const String videoIdfive = 'https://youtu.be/Jsym-0cv7CU?t=';
    const double startSecondsfive = 0000000000000000000000000001580;
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """homosexual""",
                      // alsoEnglishWord: "also: homosexual",
                      britshText:
                          """IpaUK: /ˌhəʊməˈsekʃuəl/,  /ˌhɒməˈsekʃuəl/""",
                      americanText: """IpaUS: /ˌhəʊməˈsekʃuəl/""",
...
"""),
                    const DefinitionKurdish(
                        text:
                            """١. (ھاوەڵناو) کە کەسێک حەزی سێکسی بۆ هەمان ڕەگەزی خۆی هەیە"""),
                    SentencesRow(
                      englishText:
                          """The homosexual woman shared her experiences at the event.""",
                      kurdishText:
                          """ژنە هاوڕەگەزخوازەکە باسی لە ئەزموونی کرد لە بۆنەکە.""",
                    ...
                    SentencesRow(
                      ...
                      kurdishText:
                          """خۆی وەک هاوڕەگەزخواز ناساند دوای ئەوەی دایکی مرد.""",

...
class EnglishEntryindiscriminately extends StatefulWidget {
  const EnglishEntryindiscriminately({super.key});

  @override
  State<EnglishEntryindiscriminately> createState() =>
      _EnglishEntryindiscriminatelyState();
}

class _EnglishEntryindiscriminatelyState
    extends State<EnglishEntryindiscriminately> {
  @override
  void initState() {
    super.initState();
...
1. In a random manner (- randomly, haphazardly, willy-nilly, arbitrarily, at random, every which way)
"the houses were indiscriminately scattered";
 
2. In an indiscriminate manner (- promiscuously)
""",
  );

  final String keyword = "indiscriminately";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""indiscriminately""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""The soldiers fired indiscriminately into the crowd.""");
  }
  ...
  Future<void> speaksentence2(String languageCode) async {
    ...
    await flutterTts.speak(
        """Gardeners often fertilize indiscriminately, usually to the detriment of the plants.""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const String videoIdone = 'https://youtu.be/wfUJvB3YZq0?t=';
    const double startSecondsone = 0000000000000000000000000001108;
    const String videoIdtwo = 'https://youtu.be/m6TW35oIefE?t=';
    const double startSecondstwo = 000000000000000000000000000646;
    const String videoIdthree = 'https://youtu.be/arBqhFisv7w?t=';
    const double startSecondsthree = 000000000000000000000000000421;
    const String videoIdfour = 'https://youtu.be/PlSwgM4PTNI?t=';
    const double startSecondsfour = 000000000000000000000000000711;
    const String videoIdfive = 'https://youtu.be/h66CvBSHvJs?t=';
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      ...
                      // alsoEnglishWord: "also: indiscriminately",
                      britshText: """IpaUK: /ˌɪndɪˈskrɪmɪnətli/""",
                      americanText: """IpaUS: /ˌɪndɪˈskrɪmɪnətli/""",
...
کوردی: 
"""),
// With short examples define "indiscriminately", please follow LX instructions
                    const DefinitionKurdish(
                        text:
                            """١. (ھاوەڵکار) بە شێوەیەک کە کارێک دەکرێت بەبێ ئەوەی گرنگی بدرێت بەوەی کێ زیانی پێدەگات"""),
                    SentencesRow(
                      englishText:
                          """The soldiers fired indiscriminately into the crowd.""",
                      kurdishText:
                          """سەربازەکان هەڕەمەکییانە تەقەیان کرد بەسەر خەڵکەکەدا.""",
                    ...
                    const DefinitionKurdish(
                        text:
                            """٢. (ھاوەڵکار) بە شێوەیەک کە هەنگاو بنێیت بەبێ بیرکردنەوەی وریایانە"""),
                    const DividerSentences(),
                    SentencesRow(
                      englishText:
                          """Gardeners often fertilize indiscriminately, usually to the detriment of the plants.""",
                      kurdishText:
                          """باخەوانەکان زۆرجار پیتاندن دەکەن بە هەڕەمەکییانە، زۆرجار بەوەی زیان دەگات بە ڕووەکەکان.""",

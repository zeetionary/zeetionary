...
class EnglishEntrythehomeoffice extends StatefulWidget {
  const EnglishEntrythehomeoffice({super.key});

  @override
  State<EnglishEntrythehomeoffice> createState() =>
      _EnglishEntrythehomeofficeState();
}

class _EnglishEntrythehomeofficeState extends State<EnglishEntrythehomeoffice> {
  @override
  void initState() {
    super.initState();
...
""",
  ...
  final String keyword = "the Home Office";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""the Home Office""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""She applied for a visa through the Home Office.""");
  }
  ...
  Widget build(BuildContext context) {
    // 188888880002200
    const String videoIdend = 'https://youtu.be/JFqiSr9A-Go?t=';
    const double startSecondsend = 00000000000000000000000000026;
    const String videoIdone = 'https://youtu.be/hoULXtnLpNg?t=';
    const double startSecondsone = 0000000000000000000000000001386;
    const String videoIdtwo = 'https://youtu.be/hYbl5bTs2KQ?t=';
    const double startSecondstwo = 0000000000000000000000000001725;
    const String videoIdthree = 'https://youtu.be/XPzrySKqRP0?t=';
    const double startSecondsthree = 0000000000000000000000000007;
    const String videoIdfour = 'https://youtu.be/9Hjj_L8cq00?t=';
    const double startSecondsfour = 000000000000000000000000000702;
    const String videoIdfive = 'https://youtu.be/_XHXKf2ThOQ?t=';
    const double startSecondsfive = 000000000000000000000000000576;
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """the Home Office""",
                      // alsoEnglishWord: "also: the Home Office",
                      britshText: """IpaUK: /ðə ˈhəʊm ɒfɪs/""",
                      americanText: """IpaUS: /ðə ˈhəʊm ɑːfɪs/""",
...
"""),
                    const DefinitionKurdish(
                        text:
                            """١. (ناو) ئۆفیسێکی حکومەتی بەریتانیا کە بەرپرسە لە سەپاندنی یاسا، کاری پۆلیس و زیندانەکان لە ئینگلاند و وێڵز، و ئەوەی کێ دەتوانێت بچێتە وڵاتەکەوە"""),
                    SentencesRow(
                      englishText:
                          """She applied for a visa through the Home Office.""",
                      kurdishText:
                          """داوای ڤیزای کرد لە ڕێگای ئۆفیسی نیشتیمانییەوە.""",

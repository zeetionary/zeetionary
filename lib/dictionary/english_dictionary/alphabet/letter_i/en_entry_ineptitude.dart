...
class EnglishEntryineptitude extends StatefulWidget {
  const EnglishEntryineptitude({super.key});

  @override
  State<EnglishEntryineptitude> createState() => _EnglishEntryineptitudeState();
}

class _EnglishEntryineptitudeState extends State<EnglishEntryineptitude> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: ineptitude (Derived forms: ineptitudes)
1. Unskillfulness resulting from a lack of training (- awkwardness, clumsiness, ineptness, maladroitness, slowness, cack-handedness [Brit, informal])
 
2. Having no qualities that would render it valuable or useful (- worthlessness)
"the drill sergeant's intent was to convince all the recruits of their ineptitude";
""",
  );

  final String keyword = "ineptitude";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""ineptitude""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const String videoIdend = 'https://youtu.be/AUaJ8pDlxi8?t=';
    const double startSecondsend = 000000000000000000000000000514;
    const String videoIdone = 'https://youtu.be/iGIbM0JbRlE?t=';
    const double startSecondsone = 000000000000000000000000000428;
    const String videoIdtwo = 'https://youtu.be/UJZY8e1rKGQ?t=';
    const double startSecondstwo = 000000000000000000000000000949;
    const String videoIdthree = 'https://youtu.be/hG0ROjzgEA0?t=';
    const double startSecondsthree = 0000000000000000000000000001847;
    const String videoIdfour = 'https://youtu.be/kAp9lPRbmoc?t=';
    const double startSecondsfour = 000000000000000000000000000396;
    const String videoIdfive = 'https://youtu.be/4kDgvpa5k4g?t=';
    const double startSecondsfive = 000000000000000000000000000854;
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """ineptitude""",
                      // alsoEnglishWord: "also: ineptitude",
                      britshText: """IpaUK: /ɪˈneptɪtjuːd/""",
                      americanText: """IpaUS: /ɪˈneptɪtuːd/""",
...
کوردی: بێجێ‌بوون، نابەجێ‌بوون، نەگونجاوی، نالەباری، لێنەوەشاوەیی، نەشیاوی، لێنەهاتوویی
"""),
// With short examples define "ineptitude", please follow LX instructions
                    const DefinitionKurdish(text: """١. (ناو) کەمی شارەزایی"""),
                    SentencesRow(
                      englishText:
                          """He was forced from office after a year for ineptitude and corruption.""",
                      kurdishText:
                          """لە دوای ساڵێک لە پێگەکەی دەرکرا بۆ گەندەڵی و نەشارەزایی.""",

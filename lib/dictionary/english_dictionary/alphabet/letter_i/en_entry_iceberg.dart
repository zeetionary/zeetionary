...
class EnglishEntryiceberg extends StatefulWidget {
  const EnglishEntryiceberg({super.key});

  @override
  State<EnglishEntryiceberg> createState() => _EnglishEntryicebergState();
}

class _EnglishEntryicebergState extends State<EnglishEntryiceberg> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: iceberg (Derived forms: icebergs)
1. A large mass of ice floating at sea; usually broken off of a polar glacier (- berg)
 
2. Lettuce with crisp tightly packed light-green leaves in a firm head (- crisphead lettuce, iceberg lettuce)
"iceberg is still the most popular lettuce";
""",
  );

  final String keyword = "iceberg";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""iceberg""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""The ship struck a hidden iceberg.""");
  }
  ...
  Widget build(BuildContext context) {
    // 188888880002200
    const String videoIdend = 'https://youtu.be/SEZu7K5tGxw?t=';
    ...
    const String videoIdtwo = 'https://youtu.be/T48_SrFCdaQ?t=';
    const double startSecondstwo = 000000000000000000000000000358;
    const String videoIdthree = 'https://youtu.be/oHSehKtDyoI?t=';
    const double startSecondsthree = 0000000000000000000000000001091;
    const String videoIdfour = 'https://youtu.be/48jBi86ih5Q?t=';
    const double startSecondsfour = 000000000000000000000000000323;
    const String videoIdfive = 'https://youtu.be/6ta_sftSfIw?t=';
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """iceberg""",
                      // alsoEnglishWord: "also: iceberg",
                      britshText: """IpaUK: /ˈaɪsbɜːɡ/""",
                      americanText: """IpaUS: /ˈaɪsbɜːrɡ/""",
...
کوردی: کێوەسەهۆڵ، کێفەسەهۆڵ، شاخەسەهۆڵ، کێف یان چیای سەهۆڵی
"""),
// With short examples define "iceberg", please follow LX instructions
                    const DefinitionKurdish(
                        text:
                            """١. (ناو) پارچە سەهۆڵێکی گەورە کە سەر ئاو کەوتووە"""),
                    SentencesRow(
                      englishText: """The ship struck a hidden iceberg.""",
                      kurdishText:
                          """کەشتییەکە کێشای بە پارچە سەهۆڵێکی شاراوە.""",

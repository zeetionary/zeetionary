...
class EnglishEntryignition extends StatefulWidget {
  const EnglishEntryignition({super.key});

  @override
  State<EnglishEntryignition> createState() => _EnglishEntryignitionState();
}

class _EnglishEntryignitionState extends State<EnglishEntryignition> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: ignition (Derived forms: ignitions)
1. The process of initiating combustion or catching fire
 
2. The mechanism that ignites the fuel in an internal-combustion engine (- ignition system)
 
3. The act of setting something on fire (- firing, lighting, kindling, inflammation)
""",
  );

  final String keyword = "ignition";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""ignition""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""Switch/Turn the ignition on.""");
  }
  ...
  Widget build(BuildContext context) {
    // 188888880002200
    const String videoIdend = 'https://youtu.be/toVfvRhWbj8?t=';
    const double startSecondsend = 000000000000000000000000000276;
    const String videoIdone = 'https://youtu.be/05gCLHlpzTU?t=';
    const double startSecondsone = 0000000000000000000000000001161;
    const String videoIdtwo = 'https://youtu.be/Bbi0kEaTgio?t=';
    const double startSecondstwo = 000000000000000000000000000363;
    const String videoIdthree = 'https://youtu.be/YiM9oosSvNg?t=';
    const double startSecondsthree = 000000000000000000000000000931;
    const String videoIdfour = 'https://youtu.be/H5_HVUS1l0Y?t=';
    const double startSecondsfour = 000000000000000000000000000597;
    const String videoIdfive = 'https://youtu.be/GaLdHd2J-xM?t=';
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """ignition""",
                      // alsoEnglishWord: "also: ignition",
                      britshText: """IpaUK: /ɪɡˈnɪʃn/""",
                      americanText: """IpaUS: /ɪɡˈnɪʃn/""",
...
کوردی: سووتان، گڕگرتن، داگیرسان، هەڵبوون، گرتن، گڕیان، دایسان
"""),
// With short examples define "ignition", please follow LX instructions
                    const DefinitionKurdish(
                        text:
                            """١. (ناو) سیستەمی ئەلیکترۆنی پێڕۆیەک کە وادەکات سووتەمەنی دەست بە سووتان بکات و بزوێنەرەکە دەست بە کار بکات"""),
                    SentencesRow(
                      englishText: """Switch/Turn the ignition on.""",
                      kurdishText: """سویچەکە بخە کار.""",
                      onPressedBritish: () => speaksentence1("en-GB"),
                      onPressedAmerican: () => speaksentence1("en-US"),
                    ),
                    const DividerDefinition(),
                    const DefinitionKurdish(
                        text: """٢. (ناو) دەستکردن بە سووتان"""),
                    const DividerSentences(),
                    SentencesRow(
                      englishText:
                          """The flames spread to all parts of the house within minutes of ignition.""",
                      kurdishText:
                          """بڵێسەکە بڵاوبوویەوە بە هەموو بەشی ماڵەکەدا لە چەند خولەکی گڕگرتن.""",

...
class EnglishEntryhydrofoil extends StatefulWidget {
  const EnglishEntryhydrofoil({super.key});

  @override
  State<EnglishEntryhydrofoil> createState() => _EnglishEntryhydrofoilState();
}

class _EnglishEntryhydrofoilState extends State<EnglishEntryhydrofoil> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: hydrofoil (Derived forms: hydrofoils)
1. A device consisting of a flat or curved piece (as a metal plate) so that its surface reacts to the water it is passing through (- foil)
"the fins of a fish act as hydrofoils";
 
2. A speedboat that is equipped with winglike structures that lift it so that it skims the water at high speeds (- hydroplane)
"the museum houses a replica of the jet hydrofoil that broke the record";
""",
  );

  final String keyword = "hydrofoil";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""hydrofoil""");
  }
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """hydrofoil""",
                      // alsoEnglishWord: "also: hydrofoil",
                      britshText: """IpaUK: /ˈhaɪdrəfɔɪl/""",
                      americanText: """IpaUS: /ˈhaɪdrəfɔɪl/""",
...
"""),
                    const DefinitionKurdish(
                        text:
                            """١. (ناو) بەلەمێک کە لەسەر ڕووی ئاو دەچێت کە خێرا دەچێت"""),
                    SentencesRow(
                      englishText:
                          """The hydrofoil glided smoothly over the water.""",
                      kurdishText:
                          """هایدرۆفۆیلەکە بە نەرمی بەسەر ئاوەکە دەچوو.""",

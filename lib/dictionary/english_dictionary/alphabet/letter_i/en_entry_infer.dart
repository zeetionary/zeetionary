...
class EnglishEntryinfer extends StatefulWidget {
  const EnglishEntryinfer({super.key});

  @override
  State<EnglishEntryinfer> createState() => _EnglishEntryinferState();
}

class _EnglishEntryinferState extends State<EnglishEntryinfer> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Verb: infer (Derived forms: inferring, infers, inferred)
1. (logic) reason by deduction; establish by deduction (- deduce, deduct, derive)
 
2. Draw from specific cases for more general cases (- generalize, generalise [Brit], extrapolate)
 
3. Conclude by reasoning; in logic (- deduce)
 
4. Guess correctly; solve by guessing (- guess)
"He inferred the right number of beans in the jar and won the prize";
 
5. Believe to be the case (- understand)
""",
  );

  final String keyword = "infer";
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""Much of the meaning must be inferred from the context.""");
  }
  ...
  Future<void> speaksentence3(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""It is difficult to infer anything from such evidence.""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const String videoIdend = 'https://youtu.be/3LN3__G2yGo?t=';
    const double startSecondsend = 0000000000000000000000000003;
    const String videoIdone = 'https://youtu.be/Kp_kqamkYpw?t=';
    ...
    const String videoIdthree = 'https://youtu.be/8X1xYpPmKvA?t=';
    const double startSecondsthree = 0000000000000000000000000001515;
    const String videoIdfour = 'https://youtu.be/Q1bSDnuIPbo?t=';
    const double startSecondsfour = 000000000000000000000000000119;
    const String videoIdfive = 'https://youtu.be/zhdBNVY55oM?t=';
    const double startSecondsfive = 000000000000000000000000000227;
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """infer""",
                      // alsoEnglishWord: "also: infer",
                      britshText: """IpaUK: /ɪnˈfɜː(r)/""",
                      americanText: """IpaUS: /ɪnˈfɜːr/""",
...
کوردی: 
"""),
// With short examples define "infer", please follow LX instructions
                    const DefinitionKurdish(
                        text:
                            """١. (کردار) بڕیاردانی ئەوەی شتێک ڕاستە یان گەشتن بە بۆچوونێک بە پشت بەستن بەو زانیارییانەی بەردەستن"""),
                    const AlsoEnglishckb(word: "ھەروەھا: deduce"),
                    SentencesRow(
                      englishText:
                          """Much of the meaning must be inferred from the context.""",
                      kurdishText:
                          """زۆرینەی واتاکە دەبێت لە بەستێنە هەڵبهێنجرێت.""",
                    ...
                    SentencesRow(
                      englishText:
                          """It is reasonable to infer that the government knew about these deals.""",
                      kurdishText:
                          """جێگای خۆیەتی گومان بکرێت کە حکومەت دەیزانی سەبارەت بە ڕێککەوتنەکان.""",
                    ...
                    SentencesRow(
                      ...
                      kurdishText: """سەختە هیچ لە بەڵگەی وەهاوە هەڵبهێنرێت.""",
                    ...
                    const DefinitionKurdish(
                        text:
                            """٢. (کردار) بە ناڕاستەوخۆ ئاماژەدان بەوەی کە شتێک ڕاستە"""),
                    const DividerSentences(),
                    SentencesRow(
                      englishText:
                          """Are you inferring (that) I’m not capable of doing the job?""",
                      kurdishText:
                          """ئایا ئاماژە بەوە دەکەیت کە من توانای کردنی کارەکەم نییە؟""",

...
class EnglishEntryhijack extends StatefulWidget {
  const EnglishEntryhijack({super.key});

  @override
  State<EnglishEntryhijack> createState() => _EnglishEntryhijackState();
}

class _EnglishEntryhijackState extends State<EnglishEntryhijack> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Verb: hijack (Derived forms: hijacked, hijacks, hijacking)
1. (crime) take arbitrarily or by force (- commandeer, highjack, pirate)
"The Cubans hijacked the plane and flew it to Miami";
 
2. Seize control of
"they hijacked the judicial process"

- Noun: hijack (Derived forms: hijacks)
1. Seizure of a vehicle in transit either to rob it or divert it to an alternate destination (- highjack)
""",
  );

  final String keyword = "hijack";
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""The plane was hijacked by two armed men on a flight from London to Rome.""");
  }
  ...
  Future<void> speaksentence2(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""Demonstrators fear that the march could be hijacked by extremists.""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const String videoIdone = 'https://youtu.be/bOKGmin8G7M?t=';
    const double startSecondsone = 000000000000000000000000000159;
    const String videoIdtwo = 'https://youtu.be/CHRqDzV58Vs?t=';
    const double startSecondstwo = 0000000000000000000000000001368;
    const String videoIdthree = 'https://youtu.be/QGXrZsX_1fU?t=';
    const double startSecondsthree = 00000000000000000000000000061;
    const String videoIdfour = 'https://youtu.be/YNu_Q11QFWY?t=';
    const double startSecondsfour = 000000000000000000000000000323;
    const String videoIdfive = 'https://youtu.be/0hg5n3KnrGo?t=';
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """hijack""",
                      // alsoEnglishWord: "also: hijack",
                      britshText: """IpaUK: /ˈhaɪdʒæk/""",
                      americanText: """IpaUS: /ˈhaɪdʒæk/""",
...
کوردی: 
"""),
// With short examples define "hijack", please follow LX instructions
                    const DefinitionKurdish(text: """١. (کردار) ڕفاندنی پێڕۆیەک، بە تایبەتی فڕۆکە"""),
                    SentencesRow(
                      englishText: """The plane was hijacked by two armed men on a flight from London to Rome.""",
                      kurdishText: """فڕۆکەکە لە لایەن دوو چەکدارەوە لە لەندەنەوە بۆ ڕۆما ڕفێندرا.""",
                      onPressedBritish: () => speaksentence1("en-GB"),
                      onPressedAmerican: () => speaksentence1("en-US"),
                    ),
                    const DividerDefinition(),
                    const DefinitionKurdish(text: """٢. (کردار) کۆنترۆڵکردنی شتێک بۆ بەکارهێنانی بۆ ئامانجی خۆت"""),
                    const DividerSentences(),
                    SentencesRow(
                      englishText: """Demonstrators fear that the march could be hijacked by extremists.""",
                      kurdishText: """خۆپیشاندەران ترسی ئەوەیان هەیە ڕێپێوانەکە لەلایەن توندڕەوانەوە بەلاڕێدا ببرێت.""",

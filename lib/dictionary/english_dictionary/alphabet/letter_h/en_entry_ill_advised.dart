...
class EnglishEntryilladvised extends StatefulWidget {
  const EnglishEntryilladvised({super.key});

  @override
  State<EnglishEntryilladvised> createState() => _EnglishEntryilladvisedState();
}

class _EnglishEntryilladvisedState extends State<EnglishEntryilladvised> {
  @override
  void initState() {
    super.initState();
...
1. Without careful prior deliberation or counsel (- unadvised)
"ill-advised efforts"; "it would be ill-advised to accept the offer";
""",
  );

  final String keyword = "ill-advised";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""ill-advised""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""Her remarks were ill-advised, to say the least.""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const double startSecondsend = 0000000000000000000000000001347;
    const String videoIdone = 'https://youtu.be/SjeS6gtPq8E?t=';
    const double startSecondsone = 000000000000000000000000000333;
    const String videoIdtwo = 'https://youtu.be/xi4wZJ-7ZCg?t=';
    const double startSecondstwo = 000000000000000000000000000805;
    const String videoIdthree = 'https://youtu.be/fY2CF-T05qY?t=';
    const double startSecondsthree = 00000000000000000000000000041;
    const String videoIdfour = 'https://youtu.be/f9SA25OukyM?t=';
    const double startSecondsfour = 000000000000000000000000000846;
    const String videoIdfive = 'https://youtu.be/zHfha18xbtg?t=';
    const double startSecondsfive = 000000000000000000000000000441;
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """ill-advised""",
                      // alsoEnglishWord: "also: ill-advised",
                      britshText: """IpaUK: /ˌɪl ədˈvaɪzd/""",
                      americanText: """IpaUS: /ˌɪl ədˈvaɪzd/""",
...
کوردی: 
"""),
// With short examples define "ill-advised", please follow LX instructions
                    const DefinitionKurdish(
                        text:
                            """١. (ھاوەڵناو) کە ئەگەری هەیە کێشە درووست بکات لە داهاتوودا"""),
                    SentencesRow(
                      englishText:
                          """Her remarks were ill-advised, to say the least.""",
                      kurdishText:
                          """لێدوانەکانی نەشیاوبوون بە لایەنی کەمەوە.""",

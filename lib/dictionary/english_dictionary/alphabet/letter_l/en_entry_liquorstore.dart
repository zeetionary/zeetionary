...
class EnglishEntryliquorstore extends StatefulWidget {
  const EnglishEntryliquorstore({super.key});

  @override
  State<EnglishEntryliquorstore> createState() => _EnglishEntryliquorstoreState();
}

class _EnglishEntryliquorstoreState extends State<EnglishEntryliquorstore> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: liquor store (Derived forms: liquor stores)
Usage: N. Amer
1. A store that sells alcoholic beverages for consumption elsewhere (- package store [N. Amer], off-licence [Brit], bottle shop [Austral, NZ], grogshop [Austral, informal], offie [Brit, informal], offy [Brit, informal])
""",
  );

  final String keyword = "liquor store";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""liquor store""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""The liquor store is just down the street from here.""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const String videoIdend = 'https://youtu.be/qzEoBrr9gRA?t=4';
    const double startSecondsend = 0;
    const String videoIdone = 'https://youtu.be/gjySnrspD7E?t=2742';
    const double startSecondsone = 0;
    const String videoIdtwo = 'https://youtu.be/Iyl-I0cs5t0?t=46';
    const double startSecondstwo = 0;
    const String videoIdthree = 'https://youtu.be/WanvSbJHbWI';
    const double startSecondsthree = 0;
    const String videoIdfour = 'https://youtu.be/6dv8zJiggBs?t=467';
    const double startSecondsfour = 0;
    const String videoIdfive = 'https://youtu.be/AXaPjULkpMQ?t=1679';
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """liquor store""",
                      // alsoEnglishWord: "also: liquor store",
                      britshText: """IpaUK: /ˈlɪkə stɔː(r)/""",
                      americanText: """IpaUS: /ˈlɪkər stɔːr/""",
...
کوردی: 
"""),
                    const DefinitionKurdish(text: """١. (ناو) فرۆشگایەک کە خواردنەوەی کحوولی دەفرۆشێت"""),
                    SentencesRow(
                      englishText: """The liquor store is just down the street from here.""",
                      kurdishText: """مەشرووبخانەکە هەر لە کۆتایی شەقامەکەوەیە لێرەوە.""",

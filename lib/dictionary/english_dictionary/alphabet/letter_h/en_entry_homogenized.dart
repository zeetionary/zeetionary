...
class EnglishEntryhomogenized extends StatefulWidget {
  const EnglishEntryhomogenized({super.key});

  @override
  State<EnglishEntryhomogenized> createState() =>
      _EnglishEntryhomogenizedState();
}

class _EnglishEntryhomogenizedState extends State<EnglishEntryhomogenized> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Adjective: homogenized
1. Formed by blending unlike elements especially by reducing one element to particles and dispersing them throughout another substance (- homogenised [Brit])
 
2. Made homogeneous (- homogenised [Brit])
""",
  );

  final String keyword = "homogenized";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""homogenized""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""They sell homogenized milk at the grocery store.""");
  }
  ...
  Widget build(BuildContext context) {
    // 188888880002200
    const String videoIdend = 'https://youtu.be/iht8e_HtLso?t=';
    const double startSecondsend = 000000000000000000000000000785;
    const String videoIdone = 'https://youtu.be/V4Uuxg6jmbo?t=';
    const double startSecondsone = 000000000000000000000000000328;
    const String videoIdtwo = 'https://youtu.be/GfhrEEcqKlA?t=';
    const double startSecondstwo = 000000000000000000000000000416;
    const String videoIdthree = 'https://youtu.be/MecVr3Bz4o0?t=';
    const double startSecondsthree = 0000000000000000000000000001653;
    const String videoIdfour = 'https://youtu.be/SqXa7k_cpnQ?t=';
    const double startSecondsfour = 000000000000000000000000000125;
    const String videoIdfive = 'https://youtu.be/isdaYjET9cg?t=';
    const double startSecondsfive = 00000000000000000000000000094;
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """homogenized""",
                      // alsoEnglishWord: "also: homogenized",
                      britshText: """IpaUK: /həˈmɒdʒənaɪzd/""",
                      americanText: """IpaUS: /həˈmɑːdʒənaɪzd/""",
...
کوردی: هێمۆژێنیزە، یەک‌جۆرکراو، چونیەک‌کراو، چونیەکێنراو
"""),
                    const DefinitionKurdish(
                        text:
                            """١. (ناو) شیر کە تێکدراوە بە شێوەیەک کە کرێمەکەی تێکەڵ بە بەشەکانی دیکە بووە"""),
                    SentencesRow(
                      englishText:
                          """They sell homogenized milk at the grocery store.""",
                      kurdishText:
                          """شیری تێکدراو دەفرۆشن لە فرۆشگای وردەواڵەکە.""",

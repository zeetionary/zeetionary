...
class EnglishEntryhoe extends StatefulWidget {
  const EnglishEntryhoe({super.key});

  @override
  State<EnglishEntryhoe> createState() => _EnglishEntryhoeState();
}

class _EnglishEntryhoeState extends State<EnglishEntryhoe> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: hoe (Derived forms: hoes)
1. A tool with a flat blade attached at right angles to a long handle
 
2. A sexually loose woman (- hussy [informal], prostitute, fornicatrix [informal])

- Verb: hoe (Derived forms: hoed, hoes, hoeing)
1. (farming) dig with a hoe
"He is hoeing the flower beds"
""",
  );

  final String keyword = "hoe";
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""The hoe made planting easier in the dry soil.""");
  }
  ...
  Future<void> speaksentence2(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""I hoed the ground where I wanted to plant sunflowers.""");
  }
  ...
  Widget build(BuildContext context) {
    // 188888880002200
    const String videoIdend = 'https://youtu.be/VYRH5Gn2Wbg?t=';
    const double startSecondsend = 00000000000000000000000000035;
    const String videoIdone = 'https://youtu.be/m4ciZasSrqg';
    const double startSecondsone = 0;
    const String videoIdtwo = 'https://youtu.be/Ir3eJ1t13fk?t=';
    const double startSecondstwo = 000000000000000000000000000772;
    const String videoIdthree = 'https://youtu.be/yyaUGe6QLtA?t=';
    const double startSecondsthree = 00000000000000000000000000058;
    const String videoIdfour = 'https://youtu.be/DeqEvXdrKo0?t=';
    const double startSecondsfour = 000000000000000000000000000327;
    const String videoIdfive = 'https://youtu.be/FjxWVNR3-7s?t=';
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """hoe""",
                      // alsoEnglishWord: "also: hoe",
                      britshText: """IpaUK: /həʊ/""",
                      americanText: """IpaUS: /həʊ/""",
...
کوردی: خاکڕن، پاچەکۆڵە، پاچولکە
"""),
                    const DefinitionKurdish(
                        text:
                            """١. (ناو) ئامرازێکی باخداری کە دەسکێکی درێژ و دەمێکی باریکی کانزایی هەیە، و بەکاردێت بۆ هەڵکەندی خاک و لابردنی گژوگیای زیان بەخش"""),
                    SentencesRow(
                      englishText:
                          """The hoe made planting easier in the dry soil.""",
                      kurdishText:
                          """خاکڕنەکە ناشتنی ئاسان کرد لە زەوییە وشکەکە.""",
                    ...
                    SentencesRow(
                      ...
                      kurdishText:
                          """زەوییەکەم خاکڕن کرد لەو شوێنەی کە دەمویست گوڵەبەڕۆژە بچێنم.""",

...
class EnglishEntryliquorice extends StatefulWidget {
  const EnglishEntryliquorice({super.key});

  @override
  State<EnglishEntryliquorice> createState() => _EnglishEntryliquoriceState();
}

class _EnglishEntryliquoriceState extends State<EnglishEntryliquorice> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: liquorice (Derived forms: liquorices)
Usage: Brit, Cdn (US: licorice)
1. Deep-rooted coarse-textured plant native to the Mediterranean region having blue flowers and pinnately compound leaves; widely cultivated in Europe for its long thick sweet roots (- licorice [N. Amer], Glycyrrhiza glabra)
 
2. A black candy flavoured with the dried root of the liquorice plant (- licorice [N. Amer])
""",
  );

  final String keyword = "liquorice";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""liquorice""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""Liquorice is sometimes used to flavor herbal teas.""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const String videoIdend = 'https://youtu.be/Ecm1rAbmDy4?t=1036';
    const double startSecondsend = 0;
    const String videoIdone = 'https://youtu.be/NRy_aYcIf5E?t=86';
    const double startSecondsone = 0;
    const String videoIdtwo = 'https://youtu.be/5aDOp3jOKxw?t=228';
    const double startSecondstwo = 0;
    const String videoIdthree = 'https://youtu.be/L-4bpVfoEo4?t=352';
    const double startSecondsthree = 0;
    const String videoIdfour = 'https://youtu.be/gQIT4Md8Whk?t=728';
    const double startSecondsfour = 0;
    const String videoIdfive = 'https://youtu.be/0LWzVDMsvK0?t=289';
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """liquorice""",
                      // alsoEnglishWord: "also: liquorice",
                      britshText: """IpaUK: /ˈlɪkərɪʃ/,  /ˈlɪkərɪs/""",
                      americanText: """IpaUS: /ˈlɪkərɪʃ/,  /ˈlɪkərɪs/""",
...
کوردی: 
"""),
                    const DefinitionKurdish(text: """١. (ناو) مادەیەکی ڕەش کە تامێکی توندی هەیە و لە ڕەگی ڕووەکێکەوە بەرهەم دێت، و لە دەرمان و شیرینیدا بەکاردێت"""),
                    SentencesRow(
                      englishText: """Liquorice is sometimes used to flavor herbal teas.""",
                      kurdishText: """بەلەک هەندێک جار بەکاردێت بۆ تامدان بە چای ڕووەکی.""",

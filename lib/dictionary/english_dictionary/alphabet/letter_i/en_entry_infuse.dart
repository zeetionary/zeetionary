...
class EnglishEntryinfuse extends StatefulWidget {
  const EnglishEntryinfuse({super.key});

  @override
  State<EnglishEntryinfuse> createState() => _EnglishEntryinfuseState();
}

class _EnglishEntryinfuseState extends State<EnglishEntryinfuse> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Verb: infuse (Derived forms: infused, infuses, infusing)
1. Teach and impress by frequent repetitions or admonitions (- inculcate, instill)
 
2. Fill, as with a certain quality (- impregnate, instill, tincture)
"The heavy traffic infuses the air with carbon monoxide";
 
3. Undergo the process of infusion
"the mint tea is infusing"
 
4. Let sit in a liquid to extract a flavour or to cleanse (- steep)
"infuse the blossoms in oil";
 
5. (medicine) introduce into the body through a vein, for therapeutic purposes
"Some physiologists infuses sugar solutions into the veins of animals"
""",
  );

  final String keyword = "infuse";
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""Her novels are infused with sadness.""");
  }
  ...
  Future<void> speaksentence2(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""Politics infuses all aspects of our lives.""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const String videoIdend = 'https://youtu.be/hQT7PN4Tzsg?t=';
    const double startSecondsend = 000000000000000000000000000323;
    const String videoIdone = 'https://youtu.be/NUzDLpSkQTg?t=';
    const double startSecondsone = 000000000000000000000000000112;
    const String videoIdtwo = 'https://youtu.be/Mhlpk5ZrYLM?t=';
    const double startSecondstwo = 000000000000000000000000000579;
    const String videoIdthree = 'https://youtu.be/6y8BiXC8c98?t=';
    const double startSecondsthree = 000000000000000000000000000597;
    const String videoIdfour = 'https://youtu.be/jJx-f_VZK2c?t=';
    const double startSecondsfour = 00000000000000000000000000092;
    const String videoIdfive = 'https://youtu.be/4zDZV-jGJ6w?t=';
    const double startSecondsfive = 000000000000000000000000000332;
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """infuse""",
                      // alsoEnglishWord: "also: infuse",
                      britshText: """IpaUK: /ɪnˈfjuːz/""",
                      americanText: """IpaUS: /ɪnˈfjuːz/""",
...
کوردی: 
"""),
// With short examples define "infuse", please follow LX instructions
                    const DefinitionKurdish(
                        text:
                            """١. (کردار) واکردنی ئەوەی کەسێک/شتێک خاسیەتێکی دیاریکراوی هەبێت"""),
                    SentencesRow(
                      englishText: """Her novels are infused with sadness.""",
                      kurdishText: """ڕۆمانەکانی پڕن لە دڵتەنگی.""",
                    ...
                    const DefinitionKurdish(
                        text:
                            """٢. (کردار) هەبوونی کاریگەرییەک لەسەر هەموو بەشەکانی شتێک"""),
                    const DividerSentences(),
                    SentencesRow(
                      englishText:
                          """Politics infuses all aspects of our lives.""",
                      kurdishText:
                          """سیاسەت کاردەکاتە سەر هەموو لایەنەکانی ژیانمان.""",

import 'package:flutter/material.dart';
import 'package:flutter_tts/flutter_tts.dart';
import 'package:youtube_player_iframe/youtube_player_iframe.dart';
import 'package:zeetionary/constants.dart';

// More sentences is NOT done

// Define the word "blaze" and provide five simple and short example sentences for each meaning. Use Merriam Webster style. Provide at least 5 meanings or more.

// replace blaze - /bleɪz/

enum TtsState { playing }

class EnglishEntryblaze extends StatelessWidget {
// blank divider
  EnglishEntryblaze({super.key});
// blank divider
  final FlutterTts flutterTts = FlutterTts();

  Future<void> speakblaze(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("blaze");
  }

  Future<void> speakblazes1(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("A huge fire was blazing in the fireplace.");
  }

  Future<void> speakblazes2(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("Within minutes the whole building was blazing.");
  }

  Future<void> speakblazes3(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("The sun blazed down from a clear blue sky.");
  }

  Future<void> speakblazes4(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("Her eyes were blazing with fury.");
  }

  Future<void> speakblazes5(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("The story was blazed all over the daily papers.");
  }

  Future<void> speakblazes6(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("In the distance machine guns were seen blazing.");
  }

  Future<void> speakblazes7(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("Five people died in the blaze.");
  }

  Future<void> speakblazes8(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("Dry wood makes a good blaze.");
  }

  Future<void> speakblazes9(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("The gardens are a blaze of colour.");
  }

  Future<void> speakblazes10(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("speakblazes1010");
  }

  Future<void> speakblazes11(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("speakblazes1111");
  }

  Future<void> speakblazes12(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("speakblazes1212");
  }

  Future<void> speakblazes13(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("speakblazes1313");
  }

  Future<void> speakblazes14(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("speakblazes1414");
  }

  Future<void> speakblazes15(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("speakblazes1515");
  }

  Future<void> speakblazes16(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("speakblazes1616");
  }

  Future<void> speakblazes17(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("speakblazes1717");
  }

  Future<void> speakblazes18(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("speakblazes1818");
  }

  Future<void> speakblazes19(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("speakblazes1919");
  }

  Future<void> speakblazes20(String languageCode) async {
    // DOPSUM: CHANGE speakblaze
    await flutterTts.setLanguage(languageCode);
    await flutterTts.setPitch(1.0);
    await flutterTts.setSpeechRate(0.5);
    await flutterTts.speak("speakblazes2020");
  }

  @override
  Widget build(BuildContext context) {
    return DefaultTabController(
      length: 3,
      child: Scaffold(
        appBar: const ZeetionaryAppbar(),
        body: Padding(
          padding:
              const EdgeInsets.only(left: 14, top: 4, right: 14, bottom: 4),
          child: Column(
            children: [
              SingleChildScrollView(
                child: Column(
                  children: [
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Row(
                          children: [
                            EntryTitle(word: "blaze"),
                          ],
                        ),
                        Row(
                          children: [
                            const IPAofEnglish(text: "IpaUK: /bleɪz/"),
                            CustomIconButtonBritish(
                              onPressed: () => speakblaze("en-GB"),
                            ),
                          ],
                        ),
                        Row(
                          children: [
                            const IPAofEnglish(text: "IpaUS: /bleɪz/"),
                            CustomIconButtonAmerican(
                              onPressed: () => speakblaze("en-US"),
                            ),
                          ],
                        ),
                      ],
                    ),
                  ],
                ),
              ),
              const CustomTabBar(
                tabs: [
                  UkIconForTab(),
                  KurdIconForTab(),
                  VideoIconForTab(), // 01
                ],
              ),
              Expanded(
                child: TabBarView(
                  children: [
                    const EnglishMeaning(),
                    SingleChildScrollView(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.end,
                        children: [
                          const DividerDefinition(),
                          const KurdishVocabulary(text: """
کوردی: گڕ، گڵ، گڵپە، کڵپە، بڵێسە، سۆڵە، زمانە، سنجر، نێرگە، ئاگر، ئاور، ئاورگرتن، ئاورتێبەربوون، سووتمان، سووتان، داگیرسان، ھەڵبوون، سوورەوەبوون، ھەڵچوون، بریقەدانەوە، تیشک‌دانەوە، درەوشانەوە، چروسان، شەوق‌دانەوە، ورشانەوە، ڕووناکی، شەوق
"""),
                          const DefinitionKurdish(
                              text:
                                  "١. (کردار) سووتان بە ڕووناکی زۆر و گوڕەوە"),
                          SentencesRow(
                            englishText:
                                "A huge fire was blazing in the fireplace.",
                            kurdishText:
                                "ئاگرێکی گەورە کڵپەی سەندبوو لە ئاگردانەکە.",
                            onPressedBritish: () => speakblazes1("en-GB"),
                            onPressedAmerican: () => speakblazes1("en-US"),
                          ),
                          const DividerSentences(),
                          SentencesRow(
                            englishText:
                                "Within minutes the whole building was blazing.",
                            kurdishText:
                                "لە ماوەی خولەکێکدا تەواوی بیناکە گڕی گرتبوو.",
                            onPressedBritish: () => speakblazes2("en-GB"),
                            onPressedAmerican: () => speakblazes2("en-US"),
                          ),
                          // const DividerDefinition(),
                          const DividerDefinition(),
                          const DefinitionKurdish(
                              text: "٢. (کردار) درەوشانەوە بە ڕەونەقەوە"),
                          SentencesRow(
                            englishText:
                                "The sun blazed down from a clear blue sky.",
                            kurdishText:
                                "خۆر دەدرەوشایەوە با ئاسمانێکی شینی ساماڵدا.",
                            onPressedBritish: () => speakblazes3("en-GB"),
                            onPressedAmerican: () => speakblazes3("en-US"),
                          ),
                          // const DividerSentences(),
                          const DividerDefinition(),
                          const DefinitionKurdish(
                              text: "٣. (کردار) چاوێک کە تووڕەیی پێوە دیارە"),
                          SentencesRow(
                            englishText: "Her eyes were blazing with fury.",
                            kurdishText: "چاوەکانی تووڕەییان لێ دەباری.",
                            onPressedBritish: () => speakblazes4("en-GB"),
                            onPressedAmerican: () => speakblazes4("en-US"),
                          ),
                          // const DividerSentences(),
                          const DividerDefinition(),
                          const DefinitionKurdish(
                              text:
                                  "٤. (کردار) بڵاوکردنەوەی ھەواڵ بە بەرفراوانی"),
                          SentencesRow(
                            englishText:
                                "The story was blazed all over the daily papers.",
                            kurdishText:
                                "چیرۆکەکە بە تەواوی ڕۆژنامە ڕۆژانەییەکاندا بڵاوکرایەوە.",
                            onPressedBritish: () => speakblazes5("en-GB"),
                            onPressedAmerican: () => speakblazes5("en-US"),
                          ),
                          // const DividerSentences(),
                          const DividerDefinition(),
                          const DefinitionKurdish(
                              text:
                                  "٥. (کردار) تەقاندنی فیشەک لە چەکێکەوە بە بەردەوامی"),
                          SentencesRow(
                            englishText:
                                "In the distance machine guns were seen blazing.",
                            kurdishText:
                                "لە دوورەوە ڕەشاش دەبینرا سەلەی دەکرد (فیشەکی زۆری پێکەوە دەھاویشت).",
                            onPressedBritish: () => speakblazes6("en-GB"),
                            onPressedAmerican: () => speakblazes6("en-US"),
                          ),
                          // const DividerSentences(),
                          const DividerDefinition(),
                          const DefinitionKurdish(
                              text: "٦. (ناو) ئاگرێکی گەورە"),
                          SentencesRow(
                            englishText: "Five people died in the blaze.",
                            kurdishText: "پێنج کەس لە ئاگرەکەدا مردن.",
                            onPressedBritish: () => speakblazes7("en-GB"),
                            onPressedAmerican: () => speakblazes7("en-US"),
                          ),
                          const DividerDefinition(),
                          const DefinitionKurdish(text: "٧. (ناو) گڕی ئاگر"),
                          SentencesRow(
                            englishText: "Dry wood makes a good blaze.",
                            kurdishText: "داری وشک گڕێکی باشی دەبێت.",
                            onPressedBritish: () => speakblazes8("en-GB"),
                            onPressedAmerican: () => speakblazes8("en-US"),
                          ),
                          const DividerDefinition(),
                          const DefinitionKurdish(
                              text:
                                  "٨. (ناو) نمایشی شتێک کە کاریگەرە و سەرنج ڕادەکێشێت"),
                          SentencesRow(
                            englishText: "The gardens are a blaze of colour.",
                            kurdishText:
                                "باخچەکە درەوشاناوەیەک لە ڕووناکی بوو.",
                            onPressedBritish: () => speakblazes9("en-GB"),
                            onPressedAmerican: () => speakblazes9("en-US"),
                          ),
                          // const DividerDefinition(),
                        ],
                      ),
                    ),
                    const YouTubeScroller(
                      children: [
                        YoutubeEmbeddedone(),
                        YoutubeEmbeddedtwo(),
                        YoutubeEmbeddedthree(),
                        YoutubeEmbeddedfour(),
                        YoutubeEmbeddedfive(),
                        YoutubeEmbeddedsix(),
                        YoutubeEmbeddedseven(),
                        YoutubeEmbeddedeight(),
                        YoutubeEmbeddednine(),
                        YoutubeEmbeddedten(),
                        YoutubeEmbeddedeleven(),
                        YoutubeEmbeddedtwelve(),
                        // YoutubeEmbeddedthirteen(),
                        // YoutubeEmbeddeddfourteen(),
                        // YoutubeEmbeddedfifteen(),
                        // YoutubeEmbeddeddsixteen(),
                        // YoutubeEmbeddeddseventeen(),
                        // YoutubeEmbeddeddeighteen(),
                        // YoutubeEmbeddeddnineteen(),
                        // YoutubeEmbeddedtwenty(),
                        // YoutubeEmbeddedmulti(),
                      ],
                    ),
                  ],
                ),
              ),
            ],
          ),
        ),
      ),
    );
  }
}

class EnglishMeaning extends StatefulWidget {
  const EnglishMeaning({super.key});

  @override
  State<EnglishMeaning> createState() => _EnglishMeaningState();
}

class _EnglishMeaningState extends State<EnglishMeaning> {
  FlutterTts flutterTts = FlutterTts();
  bool isSpeaking = false;

  Future<void> startSpeaking(
      String languageCode, EnglishMeaningConst englishMeaningConst) async {
    String textToSpeak = """
${englishMeaningConst.text}
""";

    await flutterTts.setLanguage(languageCode);
    await flutterTts.speak(textToSpeak);

    setState(() {
      isSpeaking = true;
    });
  }

  // Function to stop TTS
  Future<void> stopSpeaking() async {
    await flutterTts.stop();

    // Update the state to reflect that TTS is stopped
    setState(() {
      isSpeaking = false;
    });
  }

  // Create an instance of EnglishMeaningConst with the desired text
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: blaze (derived forms: blazes)
1. A strong fire that burns brightly (= blazing)
"the blaze spread rapidly";

2. A cause of difficulty and suffering (= hell)
"go to blazes";

3. Noisy and unrestrained mischief (= hell)
"raising blazes";

4. A light within the field of vision that is brighter than the brightness to which the eyes are adapted (= glare, brilliance)
"a blaze of sunlight";
 
5. A light-coloured marking
"they chipped off bark to mark the trail with blazes"; "the horse had a blaze between its eyes"

- Verb: blaze (derived forms: blazed, blazes, blazing)
1. Shine brightly and intensively
"Meteors blazed across the atmosphere"
 
2. Shoot rapidly and repeatedly (= blaze away)
"He blazed away at the men";

3. Burn brightly and intensely
"The summer sun alone can cause a pine to blaze"
 
4. Move rapidly and as if blazing (= blaze out)
"The spaceship blazed out into space";
 
5. Indicate by marking trees with blazes
"blaze a trail"
""",
  );

  @override
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const DividerDefinition(),

          EnglishButtonTTS(
            onBritishPressed: (languageCode) =>
                startSpeaking(languageCode, englishMeaningConst),
            onAmericanPressed: (languageCode) =>
                startSpeaking(languageCode, englishMeaningConst),
            onStopPressed: stopSpeaking,
          ),
          // Speaker icon for American English
          englishMeaningConst,
        ],
      ),
    );
  }
}

// DOPSUM: FIRST YOUTUBE VIDEO

class YoutubeEmbeddedone extends StatelessWidget {
  const YoutubeEmbeddedone({super.key});

  final String _videoId = 'LXhLlWQtdt0';
  final double _startSeconds = 1024;

  @override
  Widget build(BuildContext context) {
    YoutubePlayerController controller = YoutubePlayerController.fromVideoId(
      videoId: _videoId,
      startSeconds: _startSeconds,
      autoPlay: true,
      params: defaultYoutubePlayerParams,
    );

    void reloadVideo() {
      controller.loadVideoById(
        videoId: _videoId,
        startSeconds: _startSeconds,
      );
    }

    return YouTubeVideosScaffold(
      controller: controller,
      onReloadVideo: reloadVideo,
    );
  }
}

class YoutubeEmbeddedtwo extends StatelessWidget {
  const YoutubeEmbeddedtwo({super.key});

  final String _videoId = 'fxGI8r8vAFk';
  final double _startSeconds = 0;

  @override
  Widget build(BuildContext context) {
    YoutubePlayerController controller = YoutubePlayerController.fromVideoId(
      videoId: _videoId,
      startSeconds: _startSeconds,
      autoPlay: true,
      params: defaultYoutubePlayerParams,
    );

    void reloadVideo() {
      controller.loadVideoById(
        videoId: _videoId,
        startSeconds: _startSeconds,
      );
    }

    return YouTubeVideosScaffold(
      controller: controller,
      onReloadVideo: reloadVideo,
    );
  }
}

class YoutubeEmbeddedthree extends StatelessWidget {
  const YoutubeEmbeddedthree({super.key});

  final String _videoId = 'nyVOi3X53bk';
  final double _startSeconds = 247;

  @override
  Widget build(BuildContext context) {
    YoutubePlayerController controller = YoutubePlayerController.fromVideoId(
      videoId: _videoId,
      startSeconds: _startSeconds,
      autoPlay: true,
      params: defaultYoutubePlayerParams,
    );

    void reloadVideo() {
      controller.loadVideoById(
        videoId: _videoId,
        startSeconds: _startSeconds,
      );
    }

    return YouTubeVideosScaffold(
      controller: controller,
      onReloadVideo: reloadVideo,
    );
  }
}

class YoutubeEmbeddedfour extends StatelessWidget {
  const YoutubeEmbeddedfour({super.key});

  final String _videoId = 'SsuWS6yE478';
  final double _startSeconds = 457;

  @override
  Widget build(BuildContext context) {
    YoutubePlayerController controller = YoutubePlayerController.fromVideoId(
      videoId: _videoId,
      startSeconds: _startSeconds,
      autoPlay: true,
      params: defaultYoutubePlayerParams,
    );

    void reloadVideo() {
      controller.loadVideoById(
        videoId: _videoId,
        startSeconds: _startSeconds,
      );
    }

    return YouTubeVideosScaffold(
      controller: controller,
      onReloadVideo: reloadVideo,
    );
  }
}

class YoutubeEmbeddedfive extends StatelessWidget {
  const YoutubeEmbeddedfive({super.key});

  final String _videoId = 'j3DuONZb3Ik';
  final double _startSeconds = 160;

  @override
  Widget build(BuildContext context) {
    YoutubePlayerController controller = YoutubePlayerController.fromVideoId(
      videoId: _videoId,
      startSeconds: _startSeconds,
      autoPlay: true,
      params: defaultYoutubePlayerParams,
    );

    void reloadVideo() {
      controller.loadVideoById(
        videoId: _videoId,
        startSeconds: _startSeconds,
      );
    }

    return YouTubeVideosScaffold(
      controller: controller,
      onReloadVideo: reloadVideo,
    );
  }
}

class YoutubeEmbeddedsix extends StatelessWidget {
  const YoutubeEmbeddedsix({super.key});

  final String _videoId = 'Y2VlTpe4srk';
  final double _startSeconds = 523;

  @override
  Widget build(BuildContext context) {
    YoutubePlayerController controller = YoutubePlayerController.fromVideoId(
      videoId: _videoId,
      startSeconds: _startSeconds,
      autoPlay: true,
      params: defaultYoutubePlayerParams,
    );

    void reloadVideo() {
      controller.loadVideoById(
        videoId: _videoId,
        startSeconds: _startSeconds,
      );
    }

    return YouTubeVideosScaffold(
      controller: controller,
      onReloadVideo: reloadVideo,
    );
  }
}

class YoutubeEmbeddedseven extends StatelessWidget {
  const YoutubeEmbeddedseven({super.key});

  final String _videoId = 'ElQdUHSXkME';
  final double _startSeconds = 17;

  @override
  Widget build(BuildContext context) {
    YoutubePlayerController controller = YoutubePlayerController.fromVideoId(
      videoId: _videoId,
      startSeconds: _startSeconds,
      autoPlay: true,
      params: defaultYoutubePlayerParams,
    );

    void reloadVideo() {
      controller.loadVideoById(
        videoId: _videoId,
        startSeconds: _startSeconds,
      );
    }

    return YouTubeVideosScaffold(
      controller: controller,
      onReloadVideo: reloadVideo,
    );
  }
}

class YoutubeEmbeddedeight extends StatelessWidget {
  const YoutubeEmbeddedeight({super.key});

  final String _videoId = 'vntKopJeeuo';
  final double _startSeconds = 136;

  @override
  Widget build(BuildContext context) {
    YoutubePlayerController controller = YoutubePlayerController.fromVideoId(
      videoId: _videoId,
      startSeconds: _startSeconds,
      autoPlay: true,
      params: defaultYoutubePlayerParams,
    );

    void reloadVideo() {
      controller.loadVideoById(
        videoId: _videoId,
        startSeconds: _startSeconds,
      );
    }

    return YouTubeVideosScaffold(
      controller: controller,
      onReloadVideo: reloadVideo,
    );
  }
}

class YoutubeEmbeddednine extends StatelessWidget {
  const YoutubeEmbeddednine({super.key});

  final String _videoId = 'rbhx3UNpV64';
  final double _startSeconds = 80;

  @override
  Widget build(BuildContext context) {
    YoutubePlayerController controller = YoutubePlayerController.fromVideoId(
      videoId: _videoId,
      startSeconds: _startSeconds,
      autoPlay: true,
      params: defaultYoutubePlayerParams,
    );

    void reloadVideo() {
      controller.loadVideoById(
        videoId: _videoId,
        startSeconds: _startSeconds,
      );
    }

    return YouTubeVideosScaffold(
      controller: controller,
      onReloadVideo: reloadVideo,
    );
  }
}

class YoutubeEmbeddedten extends StatelessWidget {
  const YoutubeEmbeddedten({super.key});

  final String _videoId = '4hRNhosh8Lk';
  final double _startSeconds = 497;

  @override
  Widget build(BuildContext context) {
    YoutubePlayerController controller = YoutubePlayerController.fromVideoId(
      videoId: _videoId,
      startSeconds: _startSeconds,
      autoPlay: true,
      params: defaultYoutubePlayerParams,
    );

    void reloadVideo() {
      controller.loadVideoById(
        videoId: _videoId,
        startSeconds: _startSeconds,
      );
    }

    return YouTubeVideosScaffold(
      controller: controller,
      onReloadVideo: reloadVideo,
    );
  }
}

class YoutubeEmbeddedeleven extends StatelessWidget {
  const YoutubeEmbeddedeleven({super.key});

  final String _videoId = '9QcdijaKdAY';
  final double _startSeconds = 191;

  @override
  Widget build(BuildContext context) {
    YoutubePlayerController controller = YoutubePlayerController.fromVideoId(
      videoId: _videoId,
      startSeconds: _startSeconds,
      autoPlay: true,
      params: defaultYoutubePlayerParams,
    );

    void reloadVideo() {
      controller.loadVideoById(
        videoId: _videoId,
        startSeconds: _startSeconds,
      );
    }

    return YouTubeVideosScaffold(
      controller: controller,
      onReloadVideo: reloadVideo,
    );
  }
}

class YoutubeEmbeddedtwelve extends StatelessWidget {
  const YoutubeEmbeddedtwelve({super.key});

  final String _videoId = 'e50YrXo2Tes';
  final double _startSeconds = 32;

  @override
  Widget build(BuildContext context) {
    YoutubePlayerController controller = YoutubePlayerController.fromVideoId(
      videoId: _videoId,
      startSeconds: _startSeconds,
      autoPlay: true,
      params: defaultYoutubePlayerParams,
    );

    void reloadVideo() {
      controller.loadVideoById(
        videoId: _videoId,
        startSeconds: _startSeconds,
      );
    }

    return YouTubeVideosScaffoldEnd(
      controller: controller,
      onReloadVideo: reloadVideo,
    );
  }
}

// end blaze// TODO Implement this library.

...
class EnglishEntryhashtag extends StatefulWidget {
  const EnglishEntryhashtag({super.key});

  @override
  State<EnglishEntryhashtag> createState() => _EnglishEntryhashtagState();
}

class _EnglishEntryhashtagState extends State<EnglishEntryhashtag> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: hashtag (Derived forms: hashtags)
1. Metadata tag used on social network and microblogging services which makes it easier for users to find messages with a specific theme or content
""",
  );

  final String keyword = "hashtag";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""hashtag""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""I often use hashtags to search for trending topics.""");
  }
  ...
  Future<void> speaksentence2(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""Please press the hash key to continue.""");
  }
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """hashtag""",
                      // alsoEnglishWord: "also: hashtag",
                      britshText: """IpaUK: /ˈhæʃtæɡ/""",
                      americanText: """IpaUS: /ˈhæʃtæɡ/""",
...
کوردی: 
"""),
// With short examples define "hashtag", please follow LX instructions
                    const DefinitionKurdish(
                        text:
                            """١. (ناو) وشە یان دەستەواژەیەک کە نیشانەی # لە پێشییەتی کە لەسەر تۆڕە کۆمەڵایەتییەکان بەکاردێت"""),
                    SentencesRow(
                      englishText:
                          """I often use hashtags to search for trending topics.""",
                      kurdishText:
                          """زۆرجار هاشتاگ بەکاردێنم بۆ گەڕان بۆ بابەتە گەرمەکان.""",
                      onPressedBritish: () => speaksentence1("en-GB"),
                      onPressedAmerican: () => speaksentence1("en-US"),
                    ),
                    const DividerDefinition(),
                    const DefinitionKurdish(text: """٢. (ناو) نیشانەی #"""),
                    const AlsoEnglishckb(
                        word:
                            "ھەروەھا ئینگلیزیی بەریتانی: hash, hash sign، ئەنگلیزیی ئەمریکی: pound sign"),
                    SentencesRow(
                      englishText: """Please press the hash key to continue.""",
                      kurdishText:
                          """تکایە پەنجە بە دووگمەی هاش بنێ بۆ بەردەوام بوون.""",
...
const double _startSecondsend = 00000000000000000000000000039;
const String _videoIdone = 'https://youtu.be/OrnpSe4OChM?t=';
...
const String _videoIdthree = 'https://youtu.be/5IZ4WZZ6hIo?t=';
const double _startSecondsthree = 00000000000000000000000000045;
const String _videoIdfour = 'https://youtu.be/Va_XMMR4R2I?t=';

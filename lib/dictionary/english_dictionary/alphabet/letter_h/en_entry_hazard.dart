...
class EnglishEntryhazard extends StatefulWidget {
  const EnglishEntryhazard({super.key});

  @override
  State<EnglishEntryhazard> createState() => _EnglishEntryhazardState();
}

class _EnglishEntryhazardState extends State<EnglishEntryhazard> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: hazard (Derived forms: hazards)
1. A source of danger; a possibility of incurring loss or misfortune (- jeopardy, peril, risk, endangerment)
"drinking alcohol is a health hazard";
 
2. An unknown and unpredictable phenomenon that causes an event to result one way rather than another (- luck, fortune, chance)
 
3. An obstacle on a golf course

- Verb: hazard (Derived forms: hazards, hazarding, hazarded)
1. Put forward, of a guess, in spite of possible refutation (- guess, venture, pretend)
 
2. Put at risk (- venture, adventure, stake, jeopardize, jeopardise [Brit])
"I will hazard my good reputation for this";
 
3. Take a risk in the hope of a favourable outcome (- gamble, chance, risk, take chances, adventure, run a risk, take a chance)
"When you buy these stocks you are hazarding";
""",
  );

  final String keyword = "hazard";
  ...
  Future<void> speaksentence2(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""Everybody is aware of the hazards of smoking.""");
  }
  ...
  Future<void> speaksentence5(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""I would hazard that she is the sole reason we are here.""");
  }
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """hazard""",
                      // alsoEnglishWord: "also: hazard",
                      britshText: """IpaUK: /ˈhæzəd/""",
                      americanText: """IpaUS: /ˈhæzərd/""",
...
کوردی: مەترسی، ڕێکەوت، هەڵکەوت
"""),
// With short examples define "hazard", please follow LX instructions
                    const DefinitionKurdish(
                        text: """١. (ناو) شتێک کە دەکرێت مەترسیدار بێت"""),
                    SentencesRow(
                      englishText:
                          """The island group is particularly vulnerable to natural hazards.""",
                      kurdishText:
                          """گرووپە دوورگەییەکە دەکرێت بە تایبەتی بەرکەوتوو بە کارەساتی سرووشتی بێت.""",
                    ...
                    SentencesRow(
                      ...
                      kurdishText:
                          """هەموان ئاگاداری مەترسییەکانی جگەرەکێشانن.""",
                    ...
                    SentencesRow(
                      englishText:
                          """Go in September if you want to avoid the hazard of extreme heat.""",
                      kurdishText:
                          """لە ئەیلوولدا بچۆ ئەگەر دەتەوێت لە مەترسی گەرمای بەهێز دووربکەویتەوە.""",
                    ...
                    const DefinitionKurdish(
                        text:
                            """٢. (کردار) کردنی پێشنیار یان مەزەندەیەک کە دەزانیت ڕەنگە هەڵە بێت"""),
                    const DividerSentences(),
                    SentencesRow(
                      englishText:
                    ...
                    SentencesRow(
                      ...
                      kurdishText:
                          """ئەو مەزەندەیە دەکەم کە ئەو تەنها هۆکارە لێرەین.""",
                      onPressedBritish: () => speaksentence5("en-GB"),
                      onPressedAmerican: () => speaksentence5("en-US"),
                    ),
                    const DividerDefinition(),
                    const DefinitionKurdish(
                        text: """٣. (کردار) خستنە مەترسی"""),
                    const AlsoEnglishckb(word: "ھەروەھا: endanger"),
                    SentencesRow(
                      englishText:
                          """Careless drivers hazard other people's lives as well as their own.""",
                      kurdishText:
                          """شۆفێرانی نائاگا ژیانی شۆفێرانی دیکە و هی خۆیان دەخەنە مەترسییەوە.""",
...
const double _startSecondsend = 000000000000000000000000000889;
const String _videoIdone = 'https://youtu.be/0jltioeaEyY?t=';
const double _startSecondsone = 00000000000000000000000000059;
const String _videoIdtwo = 'https://youtu.be/8KmmZBBJGkE?t=';
const double _startSecondstwo = 000000000000000000000000000621;
const String _videoIdthree = 'https://youtu.be/6ta_sftSfIw?t=';
const double _startSecondsthree = 0000000000000000000000000001609;
const String _videoIdfour = 'https://youtu.be/hzAgFNh4vRY?t=';
const double _startSecondsfour = 000000000000000000000000000122;
const String _videoIdfive = 'https://youtu.be/-7ZmDHudOEM?t=';

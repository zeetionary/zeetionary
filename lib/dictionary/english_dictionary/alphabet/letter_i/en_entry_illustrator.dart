...
class EnglishEntryillustrator extends StatefulWidget {
  const EnglishEntryillustrator({super.key});

  @override
  State<EnglishEntryillustrator> createState() =>
      _EnglishEntryillustratorState();
}

class _EnglishEntryillustratorState extends State<EnglishEntryillustrator> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: illustrator (Derived forms: illustrators)
1. An artist who makes illustrations (for books, magazines or advertisements etc.)
""",
  );

  final String keyword = "illustrator";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""illustrator""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts
        .speak("""The illustrator's sketches made the story come alive.""");
  }
  ...
  Widget build(BuildContext context) {
    // 188888880002200
    const String videoIdend = 'https://youtu.be/GEhtMHLMAss?t=';
    const double startSecondsend = 000000000000000000000000000646;
    const String videoIdone = 'https://youtu.be/sz8luEznxbM?t=';
    const double startSecondsone = 00000000000000000000000000027;
    const String videoIdtwo = 'https://youtu.be/CjiKlLE3NuI';
    const double startSecondstwo = 0;
    const String videoIdthree = 'https://youtu.be/ROP45rjvOHg?t=';
    const double startSecondsthree = 000000000000000000000000000392;
    const String videoIdfour = 'https://youtu.be/EN6jtHIqeTg?t=';
    const double startSecondsfour = 000000000000000000000000000475;
    const String videoIdfive = 'https://youtu.be/Ko75zhPwKks?t=';
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """illustrator""",
                      // alsoEnglishWord: "also: illustrator",
                      britshText: """IpaUK: /ˈɪləstreɪtə(r)/""",
                      americanText: """IpaUS: /ˈɪləstreɪtər/""",
...
کوردی: نەخش‌کار، نەخش‌کێش، وێنەکێش
"""),
                    const DefinitionKurdish(
                        text:
                            """١. (ناو) کەسێک کە وێنە و نیگار دەکێشێت بۆ کتێب، هتد"""),
                    SentencesRow(
                      englishText:
                          """The illustrator's sketches made the story come alive.""",
                      kurdishText:
                          """وێنەکانی وێنەکێشەکە ڕۆحی کرد بە بەری چیرۆکەکەدا.""",

...
class EnglishEntryhob extends StatefulWidget {
  const EnglishEntryhob({super.key});

  @override
  State<EnglishEntryhob> createState() => _EnglishEntryhobState();
}

class _EnglishEntryhobState extends State<EnglishEntryhob> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: hob (Derived forms: hobs)
1. [Brit] The top of a cooker with hotplates or burners (- cooktop [N. Amer])
 
2. A shelf beside an open fire where something can be kept warm
 
3. (folklore) a small grotesque supernatural creature that makes trouble for human beings (- goblin, hobgoblin, pooka [Ireland])
 
4. (folklore) fairies that are somewhat mischievous (- elf, gremlin, pixie, pixy, brownie, imp)
 
5. A hard steel edge tool used to cut gears
 
6. A male ferret

- Verb: hob (Derived forms: hobbing, hobbed, hobs)
1. Cut with a hob
""",
  );

  final String keyword = "hob";
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""A pan of soup was simmering on the hob.""");
  }
  ...
  Widget build(BuildContext context) {
    // 188888880002200
    const String videoIdend = 'https://youtu.be/4kbD43Lt7vk?t=';
    const double startSecondsend = 000000000000000000000000000276;
    const String videoIdone = 'https://youtu.be/Xn1LUo5ra_A?t=';
    const double startSecondsone = 00000000000000000000000000050;
    const String videoIdtwo = 'https://youtu.be/ff04ecF9Dfw?t=';
    const double startSecondstwo = 0000000000000000000000000001100;
    const String videoIdthree = 'https://youtu.be/7AAhVg9cUo0?t=';
    const double startSecondsthree = 000000000000000000000000000411;
    const String videoIdfour = 'https://youtu.be/KLmH7o6xK70?t=';
    const double startSecondsfour = 00000000000000000000000000058;
    const String videoIdfive = 'https://youtu.be/twOTMvT35E4?t=';
    const double startSecondsfive = 000000000000000000000000000372;
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """hob""",
                      // alsoEnglishWord: "also: hob",
                      britshText: """IpaUK: /hɒb/""",
                      americanText: """IpaUS: /hɑːb/""",
...
"""),
                    ...
                    const AlsoEnglishckb(word: "ھەروەھا: stovetop"),
                    SentencesRow(
                      englishText:
                          """A pan of soup was simmering on the hob.""",
                      kurdishText: """قاپێک شۆربا لەسەر تەباخەکە دەکوڵا.""",

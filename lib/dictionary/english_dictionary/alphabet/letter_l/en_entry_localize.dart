...
class EnglishEntrylocalize extends StatefulWidget {
  const EnglishEntrylocalize({super.key});

  @override
  State<EnglishEntrylocalize> createState() => _EnglishEntrylocalizeState();
}

class _EnglishEntrylocalizeState extends State<EnglishEntrylocalize> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Verb: localize (Derived forms: localized, localizes, localizing)
1. Identify the location or place of (- place, localise [Brit])
"We localized the source of the infection";
 
2. Concentrate on a particular place or spot (- localise [Brit], focalize, focalise [Brit])
"The infection has localized in the left eye";
 
3. Restrict something to a particular area (- localise [Brit])
 
4. Locate (- set, localise [Brit], place)
""",
  );

  final String keyword = "localize";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""localize""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""They hope to localize the war.""");
  }
  ...
  Future<void> speaksentence2(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""Electricians worked through the night to localize the faulty switches.""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const String videoIdend = 'https://youtu.be/d92oWdkJsXA?t=1109';
    const double startSecondsend = 0;
    const String videoIdone = 'https://youtu.be/xYZMWkmXX3k?t=439';
    const double startSecondsone = 0;
    const String videoIdtwo = 'https://youtu.be/3BqCWBbkvy8?t=5041';
    const double startSecondstwo = 0;
    const String videoIdthree = 'https://youtu.be/dCXu8Ju_fdY?t=424';
    const double startSecondsthree = 0;
    const String videoIdfour = 'https://youtu.be/OXeC0EzNA-c?t=1112';
    const double startSecondsfour = 0;
    const String videoIdfive = 'https://youtu.be/SaMxjiixJ18?t=911';
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """localize""",
                      // alsoEnglishWord: "also: localize",
                      britshText: """IpaUK: /ˈləʊkəlaɪz/""",
                      americanText: """IpaUS: /ˈləʊkəlaɪz/""",
...
کوردی: 
"""),
// With short examples define "localize", please follow LX instructions
                    const DefinitionKurdish(text: """١. (کردار) سنووردارکردنی شتێک یان کاریگەرییەکانی بە ناوچەیەکی دیاریکراوەوە"""),
                    const AlsoEnglishckb(word: "ھەروەھا: confine"),
                    SentencesRow(
                      englishText: """They hope to localize the war.""",
                      kurdishText: """بەهیوان جەنگەکە سنووردار بکەن.""",
                      englishNote: """This means they aim to limit the war to a specific area or region, preventing it from spreading to other places.""",
                      onPressedBritish: () => speaksentence1("en-GB"),
                      onPressedAmerican: () => speaksentence1("en-US"),
                    ),
                    const DividerDefinition(),
                    const DefinitionKurdish(text: """٢. (کردار) دۆزینەوەی ئەوەی شتێک لە کوێیە"""),
                    const DividerSentences(),
                    SentencesRow(
                      englishText: """Electricians worked through the night to localize the faulty switches.""",
                      kurdishText: """کارەباچییەکان بە درێژایی شەو کاریان کرد بۆ ئەوەی شوێنی سویچە تێکچووەکان.""",

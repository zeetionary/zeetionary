...
class EnglishEntryliter extends StatefulWidget {
  const EnglishEntryliter({super.key});

  @override
  State<EnglishEntryliter> createState() => _EnglishEntryliterState();
}

class _EnglishEntryliterState extends State<EnglishEntryliter> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Noun: liter (Derived forms: liters)
Usage: US (elsewhere: litre)
1. A metric unit of capacity, formerly defined as the volume of one kilogram of pure water under standard conditions; now equal to 1,000 cubic centimeters (or approximately 1.75 pints) (- litre [Brit, Cdn], l, cubic decimeter [US], cubic decimetre [Brit, Cdn])
""",
  );

  final String keyword = "liter";
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""The tax increase will add 4p to a litre of petrol.""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const String videoIdend = 'https://youtu.be/Zhw9BFNpfYw?t=9';
    const double startSecondsend = 0;
    const String videoIdone = 'https://youtu.be/BaQDMeqAeAc?t=323';
    const double startSecondsone = 0;
    const String videoIdtwo = 'https://youtu.be/6wMXvP5LiiU?t=125';
    const double startSecondstwo = 0;
    const String videoIdthree = 'https://youtu.be/0rhUs0B2FMQ?t=105';
    const double startSecondsthree = 0;
    const String videoIdfour = 'https://youtu.be/qFldcigm4N4?t=389';
    const double startSecondsfour = 0;
    const String videoIdfive = 'https://youtu.be/X3TAROotFfM?t=174';
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """liter""",
                      // alsoEnglishWord: "also: liter",
                      britshText: """IpaUK: /ˈliːtə(r)/""",
                      americanText: """IpaUS: /ˈliːtər/""",
...
کوردی: 
"""),
// With short examples define "liter", please follow LX instructions
                    const DefinitionKurdish(text: """١. (ناو) یەکەیەکی پێوانی قەبارەی شل یان گاز کە یەکسانە بە ١٠٠٠ سەنتیمەتر چوارگۆشە"""),
                    SentencesRow(
                      englishText: """The tax increase will add 4p to a litre of petrol.""",
                      kurdishText: """زیادبوونی باجەکە ٤ پێنس زیاد دەکات بۆ لیترێک بەنزین.""",

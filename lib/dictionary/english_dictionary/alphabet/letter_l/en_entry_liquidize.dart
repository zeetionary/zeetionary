...
class EnglishEntryliquidize extends StatefulWidget {
  const EnglishEntryliquidize({super.key});

  @override
  State<EnglishEntryliquidize> createState() => _EnglishEntryliquidizeState();
}

class _EnglishEntryliquidizeState extends State<EnglishEntryliquidize> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Verb: liquidize (Derived forms: liquidized, liquidizes, liquidizing)
1. Sell or get rid of all one's merchandise (- sell out, sell up, liquidise [Brit])
 
2. (physics) make (a solid substance) liquid, as by heating (- liquefy, liquify, liquidise [Brit])
""",
  );

  final String keyword = "liquidize";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""liquidize""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""The chef liquidized the berries for the sauce.""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const String videoIdend = 'https://youtu.be/8ruW_XX8Z20?t=1053';
    const double startSecondsend = 0;
    const String videoIdone = 'https://youtu.be/yyKTuO3f6Zg?t=4820';
    const double startSecondsone = 0;
    const String videoIdtwo = 'https://youtu.be/kwZjknehxxQ?t=574';
    const double startSecondstwo = 0;
    const String videoIdthree = 'https://youtu.be/fFJK9HwSJ94?t=2872';
    const double startSecondsthree = 0;
    const String videoIdfour = 'https://youtu.be/7Fn6D-8UFFU?t=332';
    const double startSecondsfour = 0;
    const String videoIdfive = 'https://youtu.be/EeBGxNr38Gk?t=511';
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """liquidize""",
                      // alsoEnglishWord: "also: liquidize",
                      britshText: """IpaUK: /ˈlɪkwɪdaɪz/""",
                      americanText: """IpaUS: /ˈlɪkwɪdaɪz/""",
...
کوردی: 
"""),
                    const DefinitionKurdish(text: """١. (کردار) گوشینی میوە، سەوزە، هتد بۆ ئەوەی ببن بە شلە"""),
                    const AlsoEnglishckb(word: "ھەروەھا: purée"),
                    SentencesRow(
                      englishText: """The chef liquidized the berries for the sauce.""",
                      kurdishText: """شێفەکە گێلاسەکانی گووشی بۆ سۆسەکە.""",

...
class EnglishEntryindividually extends StatefulWidget {
  const EnglishEntryindividually({super.key});

  @override
  State<EnglishEntryindividually> createState() =>
      _EnglishEntryindividuallyState();
}

class _EnglishEntryindividuallyState extends State<EnglishEntryindividually> {
  @override
  void initState() {
    super.initState();
...
""";
  ...
  final EnglishMeaningConst englishMeaningConst = const EnglishMeaningConst(
    text: """
- Adverb: individually
1. Apart from others (- separately, singly, severally, one by one, on an individual basis)
"taken individually, the rooms were, in fact, square";
""",
  );

  final String keyword = "individually";
  ...
  Future<void> speakheadword(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""individually""");
  }
  ...
  Future<void> speaksentence1(String languageCode) async {
    ...
    await flutterTts.setPitch(ttsPitch);
    await flutterTts.setSpeechRate(ttsSpeechRate);
    await flutterTts.speak("""The manager spoke to them all individually.""");
  }
  ...
  Widget build(BuildContext context) {
    ...
    const double startSecondsone = 0000000000000000000000000001111;
    const String videoIdtwo = 'https://youtu.be/a9m3GD0DbPY?t=';
    const double startSecondstwo = 000000000000000000000000000953;
    const String videoIdthree = 'https://youtu.be/hXRst2j9P3g?t=';
    const double startSecondsthree = 000000000000000000000000000107;
    const String videoIdfour = 'https://youtu.be/Lkl9_3-jX6c?t=';
    const double startSecondsfour = 000000000000000000000000000115;
    const String videoIdfive = 'https://youtu.be/GkgXRyScubI?t=';
    const double startSecondsfive = 0000000000000000000000000001048;
                ...
                flexibleSpace: FlexibleSpaceBar(
                  background: SingleChildScrollView(
                    child: EntryPageColumn(
                      word: """individually""",
                      // alsoEnglishWord: "also: individually",
                      britshText: """IpaUK: /ˌɪndɪˈvɪdʒuəli/""",
                      americanText: """IpaUS: /ˌɪndɪˈvɪdʒuəli/""",
...
کوردی: 
"""),
// With short examples define "individually", please follow LX instructions
                    const DefinitionKurdish(
                        text:
                            """١. (ھاوەڵکار) بەشێوەیەک کە یەک بە یەک دەکرێت نەک هەموو بە یەکەوە"""),
                    SentencesRow(
                      englishText:
                          """The manager spoke to them all individually.""",
                      kurdishText:
                          """بەڕێوەبەرەکە یەک بە یەک قسەی لەگەڵ هەموویان کرد.""",
